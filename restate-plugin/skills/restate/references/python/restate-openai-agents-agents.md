...
```bash
pip install restate-sdk[openai]
```

Or with uv:

```bash
uv add "restate-sdk[openai]"
```
...
Use `DurableRunner.run(agent, message)` to run an agent durably. Decorate tool functions with `@durable_function_tool` and use `restate_context()` inside tools for durable side effects.
