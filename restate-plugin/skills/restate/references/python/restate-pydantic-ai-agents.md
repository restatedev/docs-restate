...
```bash
pip install restate-sdk[pydantic]
```

Or with uv:

```bash
uv add "restate-sdk[pydantic]"
```
...
Wrap a Pydantic AI `Agent` with `RestateAgent(agent)` for durable execution. Use `restate_context().run_typed()` inside tool functions for durable side effects.
