...
You can attach metadata to a `TerminalException`. The metadata map is propagated to callers and accessible via `TerminalException.getMetadata()` (requires restate-server >= 1.6):
