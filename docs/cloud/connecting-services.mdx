...
- Connect [serverless functions (Vercel, Cloudflare Workers, Deno Deploy, etc.) or other public endpoints](](/cloud/connecting-services#connecting-serverless-functions-and-other-public-endpoints)), by signing requests with your cloud environment's public key.
...
You can optionally encrypt data client-side means Restate Cloud can never see your data.
...
Only the following journal entries are encrypted:
...
Deploy a secure endpoint to support observability and debugging via the UI.
