...
With LangChain, you expose each specialist as a separate Restate service and call it via `restate_context().service_call()`. Restate's middleware journals the LLM response that picks the specialist, and the inter-service hop is durable: caller and callee are journaled independently.
