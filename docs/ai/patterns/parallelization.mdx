...
Restate works with any AI SDK. Wrap tool executions in `ctx.run()` to ensure durability, and use `RestatePromise.all()` (TypeScript) or `restate.gather()` (Python) to coordinate parallel execution.
...
## Parallelizing agent tasks

Execute multiple independent agent tasks simultaneously, such as analyzing different aspects of the same input or processing multiple requests concurrently.
