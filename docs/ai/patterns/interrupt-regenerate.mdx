...
The orchestrator's `message` handler loads the conversation history, cancels any in-flight task, waits for its cleanup to finish, then dispatches a fresh task and stores its invocation ID for the next interruption. The cancellation API sends a durable cancellation signal and the attach API blocks until the target has finished its cleanup; the cancelled invocation terminating with a `TerminalError` is the expected outcome.
...
On the other side, the cancellation surfaces as a terminal error at the next Restate await. A `try/catch` around the work lets you run durable cleanup before re-raising.
