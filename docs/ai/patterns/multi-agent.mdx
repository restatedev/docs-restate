...
When you need true separation of concerns, independent scaling, or want to use different technologies for different agents, deploy them as separate services. This approach gives you maximum flexibility but requires more infrastructure planning.

Remote agents run as independent services that communicate over HTTP, but Restate makes these calls look like local function calls while providing end-to-end durability and failure recovery.

**Key advantages:**
- **Independent scaling**: Scale each agent based on its specific workload
- **Technology flexibility**: Use different languages, frameworks, or AI SDKs for each agent
- **Team ownership**: Different teams can own and deploy different agents
- **Fault isolation**: Problems in one agent don't affect others
