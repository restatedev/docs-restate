...
Each step in the chain is wrapped with `ctx.run()` to ensure fault tolerance and automatic recovery. If any step fails, Restate will retry from that exact point without losing previous progress.
