...
<Step title="Send messages to a chat session">
    ...
    ```
    ```shell Python
    curl localhost:8080/Chat/user123/on_message \
      --json '{"message": "Write a poem about Durable Execution?"}'
    ```
    </CodeGroup>
        </Tab>
...
Consult the [Chat UI integration guide](/ai/patterns/chat-ui-integration) next, to learn how to call the session object from your Chat UI.