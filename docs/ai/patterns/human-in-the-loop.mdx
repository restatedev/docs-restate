...
Some agent actions need human approvals for dangerous actions: deploying code, sending money, modifying user accounts. With Restate, your agent can **pause** execution, wait on an awakeable, and **resume** exactly where it left off, even if the process restarts in the meantime.
