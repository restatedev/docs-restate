...
The agents are served as Restate services over HTTP. The agent can now be called at `http://localhost:8080/WeatherAgent/run`.
...
We use this for:
1. **Persisting LLM responses**: We use the `DurableModelCalls(restate_context)` model provider, so that every LLM response is saved in Restate Server and can be replayed during recovery.
...
To fix the problem, remove the line `fail_on_denver` from the `fetch_weather` function in the `utils.py` file:


```python {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/ai-examples/refs/heads/tour_openai/openai-agents/tour-of-agents/app/utils/utils.py#weather"} 
...
```python parallel_agents.py {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/ai-examples/refs/heads/tour_openai/openai-agents/tour-of-agents/app/parallel_agents.py?collapse_imports"} 
@function_tool
...
The OpenAI Agent SDK handles retries internally for LLM calls due to rate limits or transient errors.
...
The `DurableModelCalls` provider automatically makes LLM calls durable and handles retry logic:

```python errorhandling_fail_on_terminal_tool_agent.py
```
...
Restate's durable execution ensures that successful LLM responses are never lost, even during retries.
Once Restate's retries are exhausted, the invocation fails with a `TerminalError` and won't be retried further.

### Tool execution errors

The OpenAI Agent SDK will convert any error in tool execution into a message to the LLM, and the agent will decide how to proceed.
...
```python advanced/rollback_agent.py {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/ai-examples/refs/heads/tour_openai/openai-agents/tour-of-agents/app/advanced/rollback_agent.py#here"} 
import restate
from agents import Agent, RunConfig, Runner, function_tool, RunContextWrapper
