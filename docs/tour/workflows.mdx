...
We will be implementing a signup workflow in this tour. Let's get started by running the example code.

<Steps>
    <Step title={"Launch Restate Server"}>
        [Install the Restate Server and CLI](](/develop/local_dev)) on your machine, and launch it:
        ```bash
        restate-server
        ```
    </Step>
...
```typescript {"CODE_LOAD::ts/src/tour/workflows/workflow.ts#here"}
...
To run the workflow service, you create an endpoint to serve the workflow and listen for requests, by default on port `9080`.

<Info>
    We will implement a set of signup workflows to show different features of Restate.
</Info>
...
The `WorkflowContext` provides durable execution capabilities:
