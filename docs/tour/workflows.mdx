...
<img src="/img/tour/workflows/workflow-app-layout.png" alt="Application Structure"/>
...
Pause workflow execution while it waits for an external event using a workflow promise:
...
export const signupWithSignals = restate.workflow({
  ...
  handlers: {
    ...
    verifyEmail: async (
      ...
      // Resolve the workflow promise to continue the main workflow
...
The workflow promise survives restarts and crashes and can be recovered on another process.

You can use workflow promises to handle asynchronous events without complex message queues or external state management.

Workflow promises can be resolved before the workflow waits for them, avoiding complex synchronization issues.

<Accordion title="Try out workflow promises" icon={"laptop"}>
...
In the UI, you can see the workflow waiting for the `email-verified` workflow promise to be resolved.
<Frame>
<img src="/img/tour/workflows/signals.png" alt="Workflow Queries"/>
</Frame>

Try killing the service and restarting it. The workflow will continue waiting for the workflow promise to be resolved.

To resolve the workflow promise, **copy over the curl request from the service logs**, which looks like this:
<div className="hidden-tabs">
<Tabs>
    <Tab title="TypeScript">
        ```bash
...
You can also use workflow promises in the other direction. The run handler resolves a workflow promise, and another handler waits for its result:
...
You will get a response like `"User created."`. If the workflow promise has not been resolved yet, the request waits until it is.
</Accordion>
...
The timers example ran three operations in parallel: two timers and a workflow promise.
