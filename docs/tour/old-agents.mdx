...
_GOAL: Simple call to LLM triggered through Restate_
...
_GOAL: Simple sequence of LLM- and tool calls, fault tolerant_
...
Example Code: Doesn't use an SDK, only does an LLM call, a tool call, another LLM call.
...
```
export const claimProcessing = restate.service({
    name: "claim-agent",
    handlers: {
        run: async (ctx: Context, req: { message: string }) => {
            const response = await ctx.run("parse-claim", async () =>
                openai.responses.parse({
                    model: "gpt-4o-mini",
                    input: req.message,
                    text: { format: zodInsuranceClaim },
                })
            );
            const claim = response.output_parsed;

            if (!claim) {
                const message = "Your request is incomplete. Missing data: " + JSON.stringify(response.incomplete_details)
                await ctx.run("email", () => emailCustomer(message))
                return message
            }

            await ctx.run("create", () => addClaimToLegacySystem(claim));
            return `Claim for ${claim.amount} due to ${claim.reason} has been processed.`;
        },
    },
});

restate.endpoint().bind(claimProcessing).listen(9080);
```

## Building an Agent Loop

_GOAL: Show integration with Agent SDKs_

Code example of an Agent loop with Vercel or OpenAI SDK, possibly third tab is manual loop. The manual loop should still use an SDK to interact with LLM and get which tools should be called.



## Observing and Debugging our Agent

_GOAL: Just showing off how nice the observability is_

One super small paragraph description.

A short video showing an the execution timeline of an agent. Example can be similar to the tool using agent example from the blog post.


_bonus (collapsed): Failures and Recovery in actions: One super small paragraph description and a video where the process crashes during an step. Restart process, see completion in the UI.
...
_GOAL: Demonstrate reacting to side-input / events, suspensions while waiting_
...
_GOAL: Demonstrate Virtual Objects, state management
