...
```typescript durableexecution/agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/durableexecution/agent.ts?collapse_prequel"} 
...
```typescript chat/agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/chat/agent.ts?collapse_prequel"} 
...
The Restate SDK gives you clients to call other services durably from your agent logic.
All calls are proxied via Restate. Restate persists the call and takes care of retries and recovery.

For example, the human approval tool from the previous section can be implemented as a separate service:
```typescript orchestration/sub-workflow-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/orchestration/sub-workflow-agent.ts#wf"} 
...
```typescript orchestration/sub-workflow-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/orchestration/sub-workflow-agent.ts#here"} 
humanApproval: tool({
  description: "Ask for human approval for high-value claims.",
  inputSchema: InsuranceClaimSchema,
...
<Accordion title={"Try it out"}>
    ...
    Use `/send` to start the claim asynchronously, without waiting for the result.
    ```bash
    curl localhost:8080/ClaimEvaluationWithWorkflows/run/send \
    --json '{"prompt": "Process my hospital bill of 3000USD for a broken leg."}'
    ```
...
```typescript orchestration/multi-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/orchestration/multi-agent.ts#here"} 
const { text } = await generateText({
  model,
...
<Accordion title={"Try it out"}>
    ...
    ```bash
    curl localhost:8080/ClaimAnalysisOrchestrator/run --json '{
        "date":"2024-10-01",
        "category":"orthopedic",
...
<Warning>
    You might have noticed that our agents set `parallelToolCalls: false` in the OpenAI provider options.
...
To parallelize tool steps, implement an orchestrator tool which use `RestatePromise`  to run multiple steps in parallel.

Here is an insurance claim agent that runs multiple analyses in parallel:

```typescript parallelwork/parallel-tools-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/parallelwork/parallel-tools-agent.ts#here"}
  const { text } = await generateText({
    model,
...
You can use the same `RestatePromise` primitives to run multiple agents in parallel, and let a main orchestrator agent combine their results:


```typescript parallelwork/parallel-agents.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/parallelwork/parallel-agents.ts?collapse_prequel"} 
export default restate.service({
...
<Accordion title={"Try it out"}>
    ...
    ```bash
    curl localhost:8080/ParallelAgentClaimAnalyzer/run --json '{
        "date":"2024-10-01",
        "category":"orthopedic",
        "reason":"hospital bill for a broken leg",
        "amount":3000,
...
```typescript errorhandling/fail-on-terminal-tool-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/errorhandling/fail-on-terminal-tool-agent.ts#max_attempts_example"} 
const model = wrapLanguageModel({
  model: openai("gpt-4o"),
  middleware: durableCalls(ctx, { maxRetryAttempts: 3 }),
});
...
By default, the AI SDK will convert any error in tool execution into a message to the LLM, and the agent will decide how to proceed.
This is often desirable, as the LLM can decide to retry the tool call, use a different tool, or provide a fallback answer.

However, if you want to treat tool execution errors as permanent failures, you can use one of the following options.
...
```typescript errorhandling/fail-on-terminal-tool-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/errorhandling/fail-on-terminal-tool-agent.ts#option2"} 
const { text } = await generateText({
  model,
  tools: {
    getWeather: tool({
...
```typescript errorhandling/stop-on-terminal-tool-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/errorhandling/stop-on-terminal-tool-agent.ts#option3"} 
...
<Accordion title="Manual Agent Loop">
    ...
    - Implement custom logic between steps
    - Interact with external systems between steps
    - Handle errors in a custom way

    Here is an example of a manual agent loop that runs until a custom stopping condition is met:

    ```typescript advanced/manual-loop-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/advanced/manual-loop-agent.ts?collapse_prequel"} 
export default restate.object({
...
```
    ...
    <Accordion title="Rolling back tool executions on failure">
        ...
        Here is an example of a travel booking agent that rolls back hotel, flight and car bookings if any step fails with a terminal error (e.g. car type not available):
        ```typescript advanced/rollback-agent.ts {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/refs/heads/new_tours/typescript/tutorials/tour-of-agents-typescript/src/advanced/rollback-agent.ts#here"} 
...
```
        ...
        ```bash
        curl localhost:8080/BookingAgent/book \
