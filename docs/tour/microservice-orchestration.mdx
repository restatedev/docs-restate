...
- **Restate Services**: Your business logic, implemented as service handlers using the Restate SDK to perform durable operations.
...
Use durable timers for complex timing logic:
...
public class EmailVerification {
  ...
  public String verifyEmail(Context ctx, VerifyEmailRequest req) {
    ...
    // Race between confirmation, reminders, and timeout
    var timeout = ctx.timer(Duration.ofDays(1));
    
    while (true) {
      var reminderTimer = ctx.timer(Duration.ofHours(1));
      
      var selected = ctx.select(confirmationFuture, reminderTimer, timeout);
      
      if (selected == confirmationFuture) {
        return confirmationFuture.await() ? "success" : "failed";
      } else if (selected == timeout) {
        throw new TerminalException("Email verification timed out");
      } else {
        // Send reminder and continue
        ctx.run("send-reminder", 
          () -> emailClient.sendReminder(req.email()));
      }
    }
  }
}
...
```java
    CompletableFuture<?>[] futures = new CompletableFuture[req.subscriptions().size()];
    for (String subscription : req.subscriptions()) {
        futures[i] = SubscriptionServiceClient.fromContext(ctx)
            .send()
            .createSubscription(req.userId(), subscription);
    }
