...
|---|---|---|---|
| **State** | Stateless | Stateful per key | Stateful per workflow |
| **Concurrency** | High parallelism | Single writer per key | Single instance per key |
| **Key Features** | Durable execution, service calls | Built-in state, consistency | Promises, signals, durable sleep |
| **Best For** | Data processing, validations, API calls | User accounts, shopping carts, entities | Approvals, orchestration, multi-step flows |
...
**Use for:** User accounts, shopping carts, game entities, any business entity that needs persistent state.
...
You can mix and match these service types within the same application - they work together seamlessly to handle different aspects of your business logic.

## Service registration

When you register your service with Restate, all exported functions become available as HTTP endpoints automatically:

```bash
# Register your service
restate deployments register http://localhost:9080

# Functions become endpoints:
# /ServiceName/functionName
# /ObjectName/key/functionName
# /WorkflowName/key/functionName
```

Each service type provides different capabilities through their context objects, but all share the same fundamental durable execution guarantees that make your applications reliable by default.