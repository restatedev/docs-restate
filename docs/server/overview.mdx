...
<Note>
    Restate supports multiple metadata storage options beyond the default replicated metadata server, including etcd, DynamoDB, and object stores. These alternatives can eliminate the need to run the stateful metadata-server role. For detailed information about choosing and configuring metadata providers, see [Metadata Storage](/server/metadata).
</Note>
