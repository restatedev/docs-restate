...
Restate stores cluster coordination data in a metadata store. This page explains the available metadata storage providers and how to choose and configure the right one for your deployment.
...
The metadata store is essential for cluster coordination and must be highly available. While metadata is updated relatively infrequently compared to data operations, its availability is crucial for cluster health.

<Warning>
    Choose your metadata provider during initial deployment based on your infrastructure and requirements. While migration between providers is supported (from replicated to external stores), it requires a maintenance window and careful execution.
</Warning>
...
3. **DynamoDB** - AWS DynamoDB managed service
4. **Object Store (S3)** - AWS S3-compatible storage backend
...
- ⚠️ Requires managing the metadata-server role on some nodes
...
- Sharing metadata across multiple Restate clusters
...
Use AWS DynamoDB as a fully-managed metadata store. This simplifies operations by eliminating the need to run stateful metadata servers.
...
Use AWS S3 as a metadata store. This provides a lightweight alternative to running metadata servers while keeping all data in object storage.
...
**Requirements:**
- AWS S3 bucket (not S3-compatible alternatives)
...
- ⚠️ **Only AWS S3 is tested and supported** (MinIO not supported for metadata)
- ⚠️ Network latency to S3 directly affects cluster operations
- ⚠️ Must ensure very low-latency access (typically same region)
- ⚠️ S3 consistency model considerations

<Warning>
    For object store metadata: Only AWS S3 is currently tested and supported. Using S3-compatible alternatives like MinIO for metadata storage is not supported and may lead to correctness issues.
</Warning>
...
# Configure the metadata server type
[metadata-server]
type = "replicated"
...
Configure Restate to use AWS S3 for metadata storage:
