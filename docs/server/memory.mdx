...
| **Invoker** | 1.5 GiB | `worker.invoker.memory-limit` |
...
The invoker is the component that drives invocations by calling your service deployments. It enforces a global memory budget for the in-flight data that flows from the server to your deployments: journal entries and state read from storage.

This budget bounds the invoker's memory usage during events that would otherwise cause memory to balloon, such as a node restart or partition leadership changes, when many pending invocations are resumed at once.

```toml restate.toml
[worker.invoker]
# Global memory budget for the invoker, shared across all partitions on this node (default: 1.5 GiB)
memory-limit = "1.5 GiB"
```

**Environment variable**: `RESTATE_WORKER__INVOKER__MEMORY_LIMIT`

The limit is shared across all partitions running on a node, rather than applied per partition. To effectively disable invoker memory limiting, set this to a very large value.

### Per-Invocation Budgets

Within the global pool, each invocation receives its own memory budget. Two options control how this budget is sized:

```toml restate.toml
[worker.invoker]
# Maximum memory a single invocation may use per direction (default: message-size-limit)
per-invocation-memory-limit = "32 MiB"

# Memory reserved from the global pool before an invocation starts (default: 32 KiB)
per-invocation-initial-memory = "32 KiB"
```

**Environment variables**:
- `RESTATE_WORKER__INVOKER__PER_INVOCATION_MEMORY_LIMIT`
- `RESTATE_WORKER__INVOKER__PER_INVOCATION_INITIAL_MEMORY`

`per-invocation-memory-limit` caps how much memory a single invocation may use. Once an invocation reaches this ceiling, it waits for in-flight data to be consumed or yields back to the scheduler. If unset, it defaults to the [`message-size-limit`](/references/server-config), and if set it is clamped to that value.

`per-invocation-initial-memory` is the amount reserved from the global pool before an invocation starts. Smaller values allow more concurrent invocations but may cause more frequent round-trips to the global pool, while larger values reduce contention but limit maximum concurrency.

<Info title="Available since v1.7.0">
The invoker memory pool and its per-invocation budgets were introduced in Restate Server v1.7.0.
</Info>
...
5. **Reduce `worker.invoker.memory-limit`** if a node has many concurrent invocations transferring large journals or state
...
| `worker.invoker.memory-limit` | `1.5 GiB` | `RESTATE_WORKER__INVOKER__MEMORY_LIMIT` | Global memory budget for in-flight invoker data |
| `worker.invoker.per-invocation-memory-limit` | `message-size-limit` | `RESTATE_WORKER__INVOKER__PER_INVOCATION_MEMORY_LIMIT` | Max memory per invocation |
| `worker.invoker.per-invocation-initial-memory` | `32 KiB` | `RESTATE_WORKER__INVOKER__PER_INVOCATION_INITIAL_MEMORY` | Memory reserved from the pool before an invocation starts |
