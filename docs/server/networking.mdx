...
| Service | Socket Path |
|---------|-------------|
| Ingress | `restate-data/ingress.sock` |
| Admin | `restate-data/admin.sock` |
| Fabric | `restate-data/fabric.sock` |
...
```bash
curl --unix-socket restate-data/ingress.sock http://local/MyService/myHandler --json '{}'
```

**Example using Unix sockets with restatectl:**
```bash
restatectl -s unix:restate-data/admin.sock status
```
