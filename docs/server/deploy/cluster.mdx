...
description: "Deploy a distributed Restate cluster."
---
...
This defaults to the hostname. When restate starts, it will look in the `restate-data` directory for a folder with its node name as name, and use that data.
...
So, in principle, nodes can have identical configurations except for the node name.
