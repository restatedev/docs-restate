...
<Info>
    [Snapshots](/operate/snapshots) are essential to support safe log trimming and also allow you to set partition replication to a subset of all cluster nodes, while still allowing for fast partition fail-over to any live node. Snapshots are also necessary to add more nodes in the future.
</Info>

<Info title="Growing the cluster in the future">
    If you plan to scale your cluster over time, we strongly recommend enabling [snapshotting](/operate/snapshots).
