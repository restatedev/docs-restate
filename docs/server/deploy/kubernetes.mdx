...
However, in practice, you'll want some kind of entry point into your cluster. The simplest approach would be a DNS entry with multiple A records that round-robin resolves to all the http-ingress nodes. Kubernetes makes this easier though - you create a Service object and Kubernetes translates that into a logical entry point. The Restate operator automatically creates these services for you.

Having something intelligent in front of your ingresses is desirable because it can perform health checks and skip over nodes that aren't reporting as ready to serve, providing better reliability than simple DNS round-robin.

The specific Kubernets provider to which you deploy will likely support binding to specific native load-balancer services, for example ELB in AWS EKS.
