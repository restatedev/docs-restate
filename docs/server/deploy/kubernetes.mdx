...
2. providing a [Restate cluster configuration](/server/clusters) via the `config` field in the `RestateCluster` spec
...
<Warning>
You should only use MinIO for snapshots, not for metadata storage, as MinIO's consistency model may lead to corrupt metadata, breaking your Restate cluster.
Use the default Raft metadata store instead.
</Warning>
