...
This is useful for integrations that rely on pod-level metadata, such as:

- **GKE ComputeClass** — assign pods to a specific machine family
- **Vault agent injection** — trigger sidecar injection via annotation-based webhooks
- **Prometheus scraping** — make pods discoverable by Prometheus
- **Datadog and other observability tools** — attach metadata for automatic metrics collection
- **Custom scheduling** — set labels for node affinity or topology spread constraints
...
Both fields are optional. Existing deployments are unaffected if neither field is set.
