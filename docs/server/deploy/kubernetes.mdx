...
- [Pod lifecycle hooks, sidecars, and extra volumes](#pod-lifecycle-hooks-sidecars-and-extra-volumes) for graceful-shutdown scripts, companion containers, and configurable termination grace periods
...
You can customize the Restate pod's lifecycle and attach companion containers using fields in `spec.compute` on `RestateCluster`.

#### Lifecycle hooks

Use `spec.compute.lifecycle` to run scripts before Restate shuts down (via a `preStop` hook) or immediately after it starts (via `postStart`). This is useful for draining connections or signaling external systems.
...
spec:
  compute:
    lifecycle:
      preStop:
        exec:
          command: ["/bin/sh", "/hooks/pre-stop.sh"]
```

<Warning>
Kubernetes runs `postStart` concurrently with the container entrypoint. It is **not** a reliable pre-start hook. For strict pre-start ordering, wrap the entrypoint using `command`/`args`.
</Warning>

#### Termination grace period

`spec.compute.terminationGracePeriodSeconds` overrides the pod termination grace period (previously fixed at 60 seconds). Set it to match the time your `preStop` hook needs to complete.
...
<Note>
Setting `terminationGracePeriodSeconds: 0` terminates the pod immediately via SIGKILL, skipping any `preStop` hook.
</Note>

#### Extra volumes and volume mounts

Use `spec.compute.extraVolumes` and `spec.compute.extraVolumeMounts` to mount additional content (such as ConfigMaps containing hook scripts) into the Restate container.
...
Volume names must not collide with operator-managed volumes (`storage`, `tmp`, `config`, trusted-CA volumes, or request-signing volumes). The operator validates this up front and returns a clear error if a conflict is detected.

#### Native sidecars

Use `spec.compute.sidecars` to run companion containers (log shippers, proxies, metrics exporters) alongside Restate. Sidecars start before the Restate container and are terminated after it. The operator sets `restartPolicy: Always` automatically so they behave as native sidecars.
...
<Note>
Native sidecars require Kubernetes 1.29+. They became generally available in Kubernetes 1.33.
</Note>

#### Full example

This example shows lifecycle hooks, extra volumes, and a sidecar working together:
