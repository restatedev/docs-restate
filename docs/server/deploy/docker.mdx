...
Without `--node-name`, the node name is derived from the machine's hostname. Inside a container the hostname is the container ID, which Docker assigns per container instance, so recreating the container (for example after a config change or an image upgrade) yields a different hostname and the restarted server presents a new node identity, looking for its data under a new `restate-data/<NODE-NAME>` directory. This is why pinning the node name matters in any environment where the hostname can change. You can also set it with the `RESTATE_NODE_NAME` environment variable.
