...
- **Networking**: Application Load Balancer (ALB) for ingress
- **Security**: EKS Pod Identity for AWS service access

## EKS with Restate Operator

Create an EKS cluster with Fargate support.
The easiest way to do this is via the Restate CDK constructs library (see [CDK Deployment](#cdk-deployment) below).
...
While it is possible to deploy Restate on EC2 instances or ECS, keep in mind the increased operational overhead compared to EKS with Fargate.
There is more manual work involved in managing the instances and persistent volumes.
