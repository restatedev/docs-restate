...
Restate supports using DynamoDB as a metadata store.
...
<Step title="Create DynamoDB table">
    ...
    ```
    Use a suitable name and billing mode that works for your setup. The schema must expose a string partition key named `pk` 
</Step>
...
# aws-endpoint-url = "http://localhost:8000"  # local testing
    ...
    - When multiple clusters share the same table, you can use `key-prefix` to give them separate scopes.
    - **Running the cluster**
        - Ensure network access from the nodes to the DynamoDB endpoint
        - Make sure that the credentials allow `GetItem`, `PutItem`, and `DeleteItem` operations on the table.
    - **Operational checks**
        - Confirm the table exists before boot; the provider doesn’t create it automatically
        - Monitor the CloudWatch metrics or the local logs to see if your DynamoDB instance is getting throttled. In this case, adjust the billing mode and IAM policies as needed.
</Step>
