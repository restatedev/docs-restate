...
|------|-------------|----------|
| `balanced` | Partition store is durable when covered by a snapshot **and** at least one replica has flushed to local storage. This is the **default when a snapshot repository is configured**. | Recommended for most clusters |
| `snapshot-only` | Partition store is durable only after a snapshot has been created, regardless of local replica state. | Maximum safety, slower trimming |
| `snapshot-and-replica-set` | Partition store is durable when **all** replicas have flushed locally **and** a snapshot exists. | Strictest mode |
| `replica-set-only` | Partition store is durable when all replicas have flushed locally, regardless of snapshot state. This is the **default when no snapshot repository is configured**. | Single-node or testing only |
| `none` | Disables automatic durability tracking and trimming entirely. | Manual control scenarios |
...
- **Archived LSN**: The log position of the latest published snapshot in the object store
