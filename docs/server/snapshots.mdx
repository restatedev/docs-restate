...
A Restate cluster maintains three essential pieces of state:
- **Metadata**: Service definitions, deployment information, and cluster configuration
- **Logs**: The Bifrost log containing all events and state changes
- **Partition store**: The durable partition state databases that hold the current state of services and workflows
