...
addresses = ["PEER_ADVERTISED_ADDRESS"]
...
<Info>
    Restate now automatically detects a publicly routable IP address for the advertised address. You only need to explicitly set `advertised-address` or `advertised-host` if the auto-detection doesn't work for your environment (e.g., behind certain NAT configurations or load balancers).
</Info>
