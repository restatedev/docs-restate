...
<img src="/img/concepts/restate-app.svg" alt="Application Structure"/>
...
- **Standard patterns**: Services look and feel like normal applications with added reliability by using the Restate SDK. Restate has SDKs for TypeScript, Java, Kotlin, Python, Go, and Rust.
- **Invoke over HTTP, Kafka, or programmatically**: You can also invoke handlers via Kafka or directly through the SDK.
- **Stateless executor**: The Restate Server stores all state and execution history, so services are stateless and can be scaled horizontally.
...
For a detailed technical walkthrough of the lifecycle of a request, see the **[guide](/guides/request-lifecycle)**.
...
<AccordionGroup>
    ...
    <Accordion title="Corrupted state">
        Restate's Virtual Objects provide a single-writer-per-object consistency model, which prevents lost updates and ensures that only one handler can modify a piece of state at a time.

        State updates are journaled together with execution, so state is always consistent with the execution history.
    </Accordion>
    <Accordion title="Race conditions">
        Requests to a single Virtual Object are processed sequentially, ensuring that only one handler can modify the state at a time.
        This concurrency model prevents race conditions.
    </Accordion>
    <Accordion title="Duplicate requests">
        ...
        Requests in between Restate handlers are always deduplicated, also without an idempotency key.
    </Accordion>
...
Each call gets driven to completion with Durable Execution.
...
In addition to reliable execution and communication, Restate provides built-in state management to ensure your application state remains consistent.

The Restate Server embeds a **key-value (K/V) store** that allows you to persist application state.

You can use this store for:
- Transactional state machines (payment status, order status…)
...
You can use Restate's K/V store from Virtual Objects and Workflows.
Restate journals all state updates alongside other context actions (e.g., run, calls) in the same execution log.
...
Restate ensures only one handler can modify a piece of state at a time, preventing lost updates or other concurrency issues.

You can query state from the Restate UI.
...
This lets you run long-running workflows on FaaS platforms without paying for idle time.
