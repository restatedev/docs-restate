...
Restate supports three types of services:
- **[Basic Services](/build/services/basic-service)**: Stateless functions that process requests independently.
- **[Virtual Objects](/build/services/virtual-object)**: Stateful functions with isolated storage per object instance and single-writer semantics.
- **[Workflows](/build/services/workflow)**: Long-running coordination processes that can wait for external events and human input.

<Info>
These will be explained in more detail in the [Services](/basics/services) section. For the remainder of this page, it suffices to know they exist.
</Info>
...
The Restate Server includes an embedded **key-value store** for persisting application state.
...
- **Stateless services**: The Restate Server stores all state and execution history and supplies it together with the request to the handler. Your service deployments don't need to manage any state and can scale horizontally.
...
Restate's K/V store makes it easy to:
- Implement consistent state machines, session state, or agent context and memory.
- Run stateful logic on stateless platforms (e.g., serverless, FaaS).
- Avoid K/V stores for transient or actor-scoped data.
