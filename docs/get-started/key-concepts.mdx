...
The Restate Server includes a **key-value store** for persisting application state. Use it for user sessions, transactional state machines, agent memory, and other application state.


### How it works

- **Scoped per entity**: Virtual Objects and Workflows each get their own isolated state
- **Automatically journaled**: State updates are recorded alongside execution steps for consistency
- **Single-writer guarantee**: Only one handler can modify state at a time, preventing race conditions
- **Stateless services**: State is injected on each request, so your services remain horizontally scalable
