...
<Frame>
<img src="/img/guides/error-handling/service-settings.png" alt="Restart from prefix" width="50%" />
</Frame>
...
<CodeGroup>
```ts TypeScript {"CODE_LOAD::ts/src/guides/retries.ts#retryable"} 
...
```python Python {"CODE_LOAD::python/src/guides/retries.py#retryable"} 
...
<Accordion title="Attaching metadata to terminal errors (Java/Kotlin, requires restate-server >= 1.6)">
In Java and Kotlin, you can attach a string metadata map to a `TerminalException`. The metadata propagates to callers and is accessible via `getMetadata()` / `metadata`.
