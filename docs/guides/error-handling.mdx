...
In Java and Kotlin, you can attach a metadata map to a `TerminalException`. The metadata is propagated to callers and accessible via `TerminalException.getMetadata()`:
