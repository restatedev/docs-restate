...
In Java and Kotlin, you can attach a metadata map to a `TerminalException`. The metadata propagates to callers and is accessible via `TerminalException.getMetadata()`. Requires Restate Server >= 1.6.
...
The caller can read the metadata from the caught exception:
