...
You can also attach a metadata map to a `TerminalException` (requires Restate Server >= 1.6). The metadata is propagated to the caller and accessible via `getMetadata()`:

```java Java {"CODE_LOAD::java/src/main/java/develop/ErrorHandling.java#metadata"} 
throw new TerminalException(
    "Something went wrong",
    Map.of("correlationId", "abc123"));
```

```kotlin Kotlin {"CODE_LOAD::kotlin/src/main/kotlin/develop/ErrorHandling.kt#metadata"} 
throw TerminalException(
    "Something went wrong",
    mapOf("correlationId" to "abc123"))
```
