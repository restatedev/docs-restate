...
### Attaching metadata to terminal errors (Java/Kotlin)

<Note>
    Requires restate-server >= 1.6 and Java/Kotlin SDK >= 2.7.0.
</Note>

In Java and Kotlin, you can attach a key-value metadata map to a `TerminalException`. The metadata is propagated to callers and accessible via `TerminalException.getMetadata()`:
...
This is useful for passing structured error context (e.g. correlation IDs or domain-specific error codes) across service boundaries.
