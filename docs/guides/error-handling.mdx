...
**Attaching metadata to terminal errors (Java/Kotlin, requires Restate Server >= 1.6)**

In Java and Kotlin, you can attach a string-to-string metadata map to a `TerminalException`. The metadata is propagated to callers and accessible via `getMetadata()` (Java) or the `metadata` property (Kotlin):

<CodeGroup>
```java Java
// Throw with metadata
throw new TerminalException("Payment failed", Map.of("correlationId", "txn-123"));
...
```kotlin Kotlin
// Throw with metadata
throw TerminalException("Payment failed", mapOf("correlationId" to "txn-123"))
