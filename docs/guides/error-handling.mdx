...
In Java and Kotlin (SDK >= 2.7.0, requires restate-server >= 1.6), you can attach a metadata map to a terminal error. The metadata is propagated to the caller and is accessible via `TerminalException.getMetadata()`:
