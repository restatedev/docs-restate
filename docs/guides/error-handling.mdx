...
In the Java/Kotlin SDK, you can attach a string-to-string metadata map to a `TerminalException` (requires Restate Server >= 1.6). The metadata propagates to callers and is accessible via `TerminalException.getMetadata()`:
