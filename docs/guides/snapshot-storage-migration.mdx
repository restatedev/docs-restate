...
- **Before snapshots exist in the new repository**: Restore the original configuration pointing to the old snapshot repository, perform a rolling restart, and resume normal operations.

- **After snapshots exist in the new repository**: If no log trimming has occurred since the original repository was disabled, you can safely discard the new repository and revert the original configuration. If logs have been trimmed based on snapshot LSNs published to the new repository, follow the same process from the start to migrate to the original destination: 1) disable log trimming; 2) update snapshot destination; 3) create and verify snapshots; 4) re-enable log trimming.
