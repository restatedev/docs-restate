...
```python expandable {"CODE_LOAD::https://raw.githubusercontent.com/restatedev/ai-examples/refs/heads/main/google-adk/template/agent.py?collapse_imports"}
APP_NAME = "agents"
...
async def run(_ctx: restate.Context, req: WeatherPrompt) -> str | None:
    ...
    if not session:
        print(f"Creating new session for user {req.user_id} with session ID {req.session_id}")
