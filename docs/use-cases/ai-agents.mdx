  ...
  tools: {
    createTicket: tool({
      ...
      execute: async ({ summary }) => {
        ...
        const ticketId = await restateContext.run(`create ticket ${issue}`, () => createTicket(issue))
        await restateContext.run("log interaction", () => logInCRM(customerId, ticketId, issue));
...
- **Tool execution**: Make tool execution steps resilient Restate Context actions like `ctx.run`
