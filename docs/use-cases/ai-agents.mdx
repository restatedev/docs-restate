...
Combine the Restate SDK with popular AI frameworks like Vercel AI SDK and OpenAI Agent SDK to build agents that interact with multiple external systems reliably:
...
- **Data integrity**: Wrap CRM logging in `ctx.run` to prevent duplicate records and maintain audit trails
- **Automatic retries**: Each external service call is retried with exponential backoff until it succeeds
- **No lost progress**: Agent resumes from the last successful step, avoiding duplicate costly operations
