...
Restate helps to make your AI agents resilient and scalable, so you can build long running agents with complex interaction patterns easily.

### Resilient Agent Execution

Restate uses Durable Execution to make your agents withstand failures without losing progress.
The Restate SDK records the steps the agent executes and recovers them if the process crashes or is restarted.
...
Restate lets you build agents any way you prefer:

<AccordionGroup>
    <Accordion title={"Integrate with AI SDKs to make brittle agents fault-tolerant"}>
    ...
    <Accordion title={"Build resilient AI workflows with unrestricted control flow"}>
