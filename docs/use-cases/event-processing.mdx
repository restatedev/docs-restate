...
async def process_post(ctx: restate.ObjectContext, post: SocialMediaPost):
    ...
    post_id = await ctx.run_typed("create post", create_post, user_id=user_id, post=post)

    while await ctx.run_typed("get status", get_post_status, post_id=post_id) == Status.PENDING:
