...
Every workflow execution is automatically traced and observable in the Restate UI:

- **Execution History**: Timeline with each workflow step
- **State Inspection**: View workflow state at any point in time
- **Error Tracking**: Error messages and rollback paths

### Single-binary Deployment

Unlike systems that require orchestrator servers, worker instances, and external databases, the Restate Server runs as a single binary alongside your application code.

This simplified architecture greatly reduces operational complexity.

For high-availability deployments, you can run multiple Restate Server instances in a cluster.
Have a look at the [Clusters](/deploy/server/cluster) section to learn how to set up a Restate cluster.

<Card title="Restate Cloud" href={"https://restate.dev/cloud/"}>
    Restate Cloud gives you a fully managed Restate Server.
    It handles all the infrastructure, scaling, and upgrades for you.
</Card>
