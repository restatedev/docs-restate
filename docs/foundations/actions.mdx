...
| **Awakeable** | A specialized, one-shot signal with a generated unique ID. It can be resolved or rejected once through an SDK or Restate's HTTP API. | Wait for an external approval callback |
