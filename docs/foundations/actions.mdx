...
| **Awakeable** | A one-shot coordination primitive with a generated unique ID. It can be resolved or rejected once through an SDK or Restate's HTTP API. | Wait for an external approval callback |
| **Workflow promise** | A named value scoped to a workflow key. It can be resolved or rejected once, then retrieved multiple times by all handlers of the workflow during workflow retention. | Share an approval between workflow handlers |
