...
<Tabs>
    ...
    <Tab title="Docker">
        ...
        The published ports are all owned by the Restate Server:

        | Port   | Owned by       | Purpose                                                     |
        | ------ | -------------- | ----------------------------------------------------------- |
        | `8080` | Restate Server | Ingress — clients call handlers here                        |
        | `9070` | Restate Server | Admin API + UI — register deployments, introspect           |
        | `9071` | Restate Server | Admin API over TLS                                          |

        Your own service code (the SDK) runs on your host on a separate port — `9080` by convention in the docs examples, but you can pick any port. Register it with `restate deployments register http://host.docker.internal:<your-port>`. Clients still call Restate on `8080`; Restate then calls your service on the registered port. See [Networking](/server/networking) for the full port reference.
