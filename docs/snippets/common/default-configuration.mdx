...
Every option below carries two badges: `toml:` is the key to write in your configuration file, and `env:` is the [environment variable](/server/configuration#environment-variables) that overrides it. Both are given in full, so you do not have to reconstruct the path from the surrounding sections. Click either badge to copy it.
