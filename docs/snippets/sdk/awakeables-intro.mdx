...
| **Awakeable** | Generated unique ID | Resolved or rejected once | External callbacks and one-shot task tokens |
...
<Note>
Signals require Restate Server 1.7 or newer. On Restate Server 1.7, set `RESTATE_EXPERIMENTAL_ENABLE_PROTOCOL_V7=true`.
</Note>

## Signals

Signals deliver durable notifications to an ongoing invocation.

A signal is identified by the target invocation ID and a name. The same named signal can be resolved multiple times. Each `await` of the signal receives the next resolution.

Each signal resolution is an independent durable notification.
