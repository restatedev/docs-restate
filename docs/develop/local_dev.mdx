...
The Restate Server is a single self-contained binary—no external dependencies required.
...
<AccordionGroup>
    ...
    <Accordion title={"Cancel a single or batch of invocations"}>
      Use `--kill` to force kill.
      ```shell
      restate invocation cancel my_invocation_id
      # also works with MyObject/myObjectKey/myHandler or a subset
      ```
...
See also the [introspection page](/operate/introspection) for more CLI debugging commands.

## Advanced: Operating clusters with `restatectl`
