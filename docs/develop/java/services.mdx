...
- [JDK](https://whichjdk.com/) >= 17 (JDK >= 23 required for the latest Restate features; JDK >= 25 recommended)

<Note title="Codegen API is deprecated">
    The annotation-processor-based **codegen API** (`sdk-api-gen` / `sdk-api-kotlin-gen`, handlers taking a `Context` parameter) is **deprecated** as of v2.9.0 and will be removed in a future release.

    Prefer the **new API**: no annotation processor, handlers drop the `Context` parameter, and you call `Restate.*` static methods (Java) or the top-level functions in `dev.restate.sdk.kotlin` (Kotlin) instead.

    The annotations (`@Service`, `@Handler`, …) are unchanged. Both styles coexist, so you can migrate one service at a time.
    See the [migration guide](https://github.com/restatedev/sdk-java/blob/main/MIGRATION.md) for step-by-step instructions.
</Note>
...
<Note>
    On JDK >= 23, the SDK runs its core logic through a native shared library (via Panama/FFM), which is required for the latest Restate features. To silence the native-access warning printed at startup, enable native access by passing `--enable-native-access=ALL-UNNAMED` as a JVM argument, or by setting `Enable-Native-Access: ALL-UNNAMED` in your JAR manifest.

    On JDK < 23, the SDK falls back to a pure-Java state machine, which is **deprecated**. Upgrading to JDK >= 25 is recommended.
</Note>
