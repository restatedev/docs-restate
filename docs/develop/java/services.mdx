...
<Accordion title="Kotlin: apply the all-open compiler plugin">
    ...
    Alternatively, define your services as interfaces and bind the implementations.
</Accordion>
...
* Handlers no longer take a `Context` parameter. Access Restate's capabilities (state, calls, side effects, timers, ...) through the static methods on the [`Restate`](https://restatedev.github.io/sdk-java/javadocs/dev/restate/sdk/Restate.html) class (Java), or the top-level functions in the `dev.restate.sdk.kotlin` package (Kotlin).
