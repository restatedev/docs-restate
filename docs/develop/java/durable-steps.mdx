...
`ctx.random()` returns the **same `RestateRandom` instance** for the lifetime of each invocation. Calling it multiple times in the same handler always returns the same object, so the sequence of generated values is stable and replay-safe.
