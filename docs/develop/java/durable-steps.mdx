...
<Info>
`ctx.random()` returns the **same `RestateRandom` instance** for the lifetime of an invocation. If you upgraded from a version before v2.7.0, redeploy your service as a new version to avoid replay mismatches on handlers that call `ctx.random()` multiple times. See the [versioning guide](/services/versioning).
</Info>
