...
`ctx.random()` returns the **same `RestateRandom` instance** for the lifetime of the invocation. Calling it multiple times within a handler does not consume additional random state.

<Warning>
  **Behavior change in SDK v2.7.0:** Prior to v2.7.0, `ctx.random()` created a new `RestateRandom` instance on every call, which changed the output ordering when called multiple times. If you call `ctx.random()` multiple times in the same handler, redeploy your service as a **new version** after upgrading to v2.7.0. See the [Restate versioning policy](/services/versioning) to avoid replay mismatches.
</Warning>
