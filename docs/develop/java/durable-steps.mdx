...
`Restate.instantNow()` (Java) / `Clock.Restate.now()` (Kotlin) returns a timestamp that is stable across retries and replays.

Use this instead of `Instant.now()` or `System.currentTimeMillis()` when you need the current time inside a handler. Calling the system clock directly would return a different value on each retry, which can cause non-deterministic behavior.
...
<Note>
    The Kotlin `Clock.Restate` API requires opting in to the [Kotlin experimental time APIs](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.time/-experimental-time/).
</Note>
