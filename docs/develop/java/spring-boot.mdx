...
You can also set **global defaults** that apply to all services by using the top-level `restate.*` properties. Per-service `restate.components.<Name>.*` properties override them:
...
# Overrides just for OrderWorkflow
restate.components.OrderWorkflow.journal-retention=PT72H
restate.components.OrderWorkflow.ingress-private=true
```
...
The HTTP server runs on multiple event loops by default. To tune the number of event loops, set:

```properties
restate.sdk.http.eventLoops=4
```
