...
<Note>Signals require Java/Kotlin SDK 2.9 or newer.</Note>
...
An awakeable is a one-shot coordination primitive with a generated unique ID. Use it when an external system needs a task token that it can resolve or reject through Restate.
...
- You address it through a workflow key and promise name instead of a generated ID
- You can retrieve its result multiple times
- Its lifetime is tied to the workflow retention period
