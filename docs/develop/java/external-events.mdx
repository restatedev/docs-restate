...
Call the signal API each time you want to wait for the next resolution of a named signal:
...
An awakeable is a specialized, one-shot signal with a generated unique ID. It provides a convenient task token that an external system can resolve or reject through Restate.
