...
<Accordion title="Virtual threads">

On Java 21+, Java handlers use virtual threads by default. Kotlin handlers continue to use `Dispatchers.Default`. To use a custom executor:
...
<Accordion title="Configuring the HTTP server event loops">

The Vert.x HTTP server runs on multiple event loops by default. To tune the number, set the `RESTATE_SDK_HTTP_EVENT_LOOPS` environment variable or the `dev.restate.sdk.http.eventLoops` system property:

```bash
RESTATE_SDK_HTTP_EVENT_LOOPS=4 java -jar myapp.jar
```

For Spring Boot, use the `restate.sdk.http.eventLoops` property in `application.properties`.
