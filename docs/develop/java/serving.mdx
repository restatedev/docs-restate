...
When using the Restate Spring Boot integration, you can configure services directly from `application.properties` or `application.yml`.

### Global default configuration

Top-level `restate.*` properties act as defaults applied to all registered services. Per-service `restate.components.<Name>.*` properties override them on a per-service basis:
...
The following properties are supported as global defaults:
`documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `retry-policy`.

Per-service and per-handler overrides continue to work as before:
...
restate.components.MyService.inactivity-timeout=PT30S
restate.components.MyService.retry-policy.initial-interval=PT0.1S
...
You can also configure a service by specifying a `RestateComponentProperties` bean in the `configuration` attribute of the `@RestateService`, `@RestateVirtualObject`, or `@RestateWorkflow` annotation:
...
@Bean
fun myServiceConfig(): RestateComponentProperties =
...
Per-service and annotation-based settings take precedence over global defaults.
