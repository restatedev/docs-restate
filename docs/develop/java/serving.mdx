...
### Configuring event loops

The HTTP server runs on multiple event loops by default (one per available CPU core). You can override this if you need to tune parallelism:

- **Spring Boot:** set `restate.sdk.http.eventLoops=<n>` in `application.properties`
- **Vanilla SDK:** set the `RESTATE_SDK_HTTP_EVENT_LOOPS=<n>` environment variable, or the `dev.restate.sdk.http.eventLoops` JVM system property
