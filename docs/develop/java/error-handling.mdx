...
You can attach a metadata map to a `TerminalException`. The metadata is propagated to callers and is accessible via `TerminalException.getMetadata()`. This requires Restate Server >= 1.6.

<CodeGroup>
```java Java
throw new TerminalException("Something went wrong", Map.of("correlationId", "abc123", "userId", "user-456"));
```
```kotlin Kotlin
throw TerminalException("Something went wrong", mapOf("correlationId" to "abc123", "userId" to "user-456"))
```
</CodeGroup>

On the caller side, you can read the metadata from the caught exception:
