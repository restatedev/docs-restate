...
You can attach a metadata map to a `TerminalException` to pass structured context back to the caller. The metadata is propagated across RPCs and accessible via `TerminalException.getMetadata()`. Requires restate-server >= 1.6.
