...
You can attach a metadata map to a `TerminalException` to propagate structured context to callers. The metadata is accessible via `getMetadata()` on the caught exception.
...
The caller can then read the metadata:
