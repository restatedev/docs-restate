...
You can attach a key-value metadata map to a `TerminalException`. This metadata is propagated to callers and accessible via `TerminalException.getMetadata()`:
...
On the receiving end, callers can inspect the metadata map:
...
try {
  ...
  // Handle the error, using the metadata for context
  throw e;
}
```
...
Metadata is useful for passing structured error context (e.g. correlation IDs, error codes, or domain-specific attributes) across service boundaries without encoding them into the error message string.