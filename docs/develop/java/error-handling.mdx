...
You can attach a key-value metadata map to a `TerminalException`. The metadata is propagated to callers and accessible via `TerminalException.getMetadata()`:

<CodeGroup>
```java Java
throw new TerminalException(
    "Something went wrong",
    Map.of("correlationId", "abc123", "errorCode", "PAYMENT_FAILED"));
```
```kotlin Kotlin
throw TerminalException(
    "Something went wrong",
    mapOf("correlationId" to "abc123", "errorCode" to "PAYMENT_FAILED"))
```
</CodeGroup>

When catching the exception, read the metadata with `getMetadata()`:

<CodeGroup>
```java Java
try {
    paymentService.charge(amount).await();
...
Use metadata to attach structured context like correlation IDs, error codes, or diagnostic information that callers need to handle the error correctly.
