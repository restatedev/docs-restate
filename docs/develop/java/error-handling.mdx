...
You can attach a metadata map to a `TerminalException`. The metadata is propagated to callers and accessible via `TerminalException.getMetadata()`.
...
When calling a service, you can retrieve the metadata on the caught exception:
