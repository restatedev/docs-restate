...
You can attach a metadata map to a `TerminalException`. The metadata is propagated to the caller and accessible via `getMetadata()` (Java) or `.metadata` (Kotlin).
...
**Reading metadata at the caller:**
