...
You can attach a metadata map to a `TerminalException`. The metadata is propagated to callers and is accessible via `TerminalException.getMetadata()`. This is useful for including structured context such as correlation IDs or error codes alongside the error message.
...
On the caller side, you can read the metadata from the caught `TerminalException`:
