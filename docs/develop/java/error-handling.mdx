...
You can attach a metadata map to a `TerminalException` to provide structured context about the failure. The metadata is propagated to callers and is accessible via `TerminalException.getMetadata()`:
...
When a caller catches the exception, it can read the metadata:
...
<Info>
    Error metadata requires Restate server >= 1.6.
</Info>
