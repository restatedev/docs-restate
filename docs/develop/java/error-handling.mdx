...
You can attach a string-to-string metadata map to a `TerminalException`. The metadata is propagated to callers and is accessible via `TerminalException.getMetadata()`. This is useful for passing structured context alongside an error (for example, a correlation ID or machine-readable error codes).
...
On the receiving end, you can read the metadata from the caught exception:
