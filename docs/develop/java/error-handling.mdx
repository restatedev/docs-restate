...
You can attach a metadata map to a `TerminalException`. The metadata is propagated to callers and is accessible via `TerminalException.getMetadata()`. This is useful for passing structured context about the failure — for example, correlation IDs or error codes — to the caller without encoding them into the error message.
...
On the caller side, retrieve the metadata from the caught exception:
