...
Pass a `RestateContainer` as the second argument to customize the test container behavior:

```typescript {"CODE_LOAD::ts/src/develop/testing.test.ts#container_options"} 
import {
...
- `withAlwaysReplay()` — forces Restate to replay all invocations from the journal on every attempt. Use this to test that your handlers are deterministic and handle replays correctly.
- `withDisableRetries()` — disables automatic retries so failures surface immediately in your tests. Useful when testing error handling and you want failures to propagate without Restate retrying them.
