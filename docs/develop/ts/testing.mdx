...
Two options on `RestateTestEnvironment.start` help you test edge cases in your handler logic.
...
Disables Restate's built-in retry policy so that failures surface immediately instead of hanging through retry backoff. Useful when testing error paths in your handlers:
