...
`RestateTestEnvironment.start` accepts two options that help surface hard-to-find bugs:

- **`alwaysReplay`** — forces the Restate server to replay after every suspension point. This surfaces non-determinism bugs where your handler produces different results on retry (for example, using `Math.random()` outside `ctx.run`).
- **`disableRetries`** — disables retry backoff in the Restate invoker. Failures surface immediately instead of hanging through exponential backoff, making test runs faster.
