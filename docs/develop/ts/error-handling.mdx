...
Beyond [retry policies](/services/configuration#retries), you can explicitly pause an invocation from within a `ctx.run` closure using `PauseError`.
A paused invocation stops retrying and stays in the `paused` state until you [manually resume it](/services/invocation/managing-invocations#resume).

Use this when you want to halt processing until a human or operator resolves an issue:
