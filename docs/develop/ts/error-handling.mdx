...
You can do this using the `asTerminalError` option at the service or handler level.

For example, to fail with `TerminalError` for each `MyValidationError`, do the following at the service level:
...
You can also set `asTerminalError` at the handler level to override the service-level mapping for a specific handler:

```typescript {"CODE_LOAD::ts/src/develop/error_handling.ts#as_terminal_handler"} 
const greeter = restate.service({
  name: "greeter",
  handlers: {
...
Handler-level options take precedence over service-level options.