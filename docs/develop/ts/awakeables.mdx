...
To save costs on FaaS deployments, Restate lets the handler suspend while awaiting the promise, and invokes it again when the result is available.
...
3. **Wait for completion** - Your handler suspends until the external system responds
4. **Resume with result** - Handler continues with the returned data

<Info>
Note that if you wait for an awakeable in an [exclusive handler](/develop/ts/overview#exclusive-vs-shared-handlers) in a Virtual Object, all other calls to this object will be queued.
</Info>
...
- External handlers call `ctx.promise("name").resolve(data)`
