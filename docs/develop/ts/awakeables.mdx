...
1. **Create an awakeable** with [`ctx.awakeable()`](https://docs.restate.dev/tsdocs/interfaces/_restatedev_restate-sdk.Context#awakeable)
   This returns a unique string identifier and a Promise.
...
- **Resolve** with [`ctx.resolveAwakeable()`](https://docs.restate.dev/tsdocs/interfaces/_restatedev_restate-sdk.Context#resolveawakeable):
```ts {CODE_LOAD::../snippets/ts/src/develop/awakeable.ts#resolve} 
ctx.resolveAwakeable(id, "hello");
```

- **Reject** with [`ctx.rejectAwakeable()`](https://docs.restate.dev/tsdocs/interfaces/_restatedev_restate-sdk.Context#rejectawakeable):
