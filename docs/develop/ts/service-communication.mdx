...
```
    ...
    <Accordion title="Option 4: Declare a typed interface with restate-sdk-gen">
        The [`@restatedev/restate-sdk-gen`](https://www.npmjs.com/package/@restatedev/restate-sdk-gen) package provides a generator-based programming model with first-class support for interface/implementation separation via the `iface` namespace.

        Define the interface once — it contains only type information and can be shared across packages without any implementation dependency:
...
```
        ...
        Implement the interface separately using `implement()`:

```ts {"CODE_LOAD::ts/src/develop/sdk-gen/iface.ts#iface_implement"} 
import { implement, state } from "@restatedev/restate-sdk-gen";
...
```
        ...
        Pass the interface directly to `client()` — no implementation import required:

```ts {"CODE_LOAD::ts/src/develop/sdk-gen/iface.ts#iface_call"} 
import { client } from "@restatedev/restate-sdk-gen";
...
```
        ...
        Use `iface.schemas()` to attach [Standard Schema](https://standardschema.dev/) validation (Zod, TypeBox, Valibot, …):

```ts {"CODE_LOAD::ts/src/develop/sdk-gen/iface.ts#iface_schemas"} 
const CreateUserReq = z.object({ username: z.string().min(3) });
