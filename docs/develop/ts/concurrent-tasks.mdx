...
Use `RestatePromise.resolve` and `RestatePromise.reject` to create `RestatePromise` instances that are already completed.
This is useful when you need to return a `RestatePromise` from a helper function but the result is already known:

```typescript {"CODE_LOAD::ts/src/develop/journaling_results.ts#resolve_reject"} 
...
## Transforming promise results with `.map()`

Use `.map()` to transform the result of a `RestatePromise` without awaiting it immediately:

```typescript {"CODE_LOAD::ts/src/develop/journaling_results.ts#map"} 
...
The mapper runs **exactly once**, regardless of how many times you await the resulting promise.
