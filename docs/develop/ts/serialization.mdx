...
## Default serde on ingress clients

When invoking services from outside a Restate handler using the [ingress client](/services/invocation/clients/typescript-sdk), you can set a default serde on the connection. This applies to all calls made through that client — handler invocations, workflow attach/output, awakeable resolution, and result retrieval — unless overridden per call.
