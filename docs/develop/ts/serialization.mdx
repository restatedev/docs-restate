...
When using the [TypeScript SDK clients](/services/invocation/clients/typescript-sdk) to invoke services from outside a handler, you can set a connection-level default serde via the `serde` option in `ConnectionOpts`. This avoids passing the serde on every individual call when working with services that use non-JSON encodings.

See the [custom serialization section](/services/invocation/clients/typescript-sdk#custom-serialization) of the SDK clients page for details.
