...
When invoking services from outside a Restate handler using the ingress client (`@restatedev/restate-sdk-clients`), you can set a default serde for an entire connection or override it per operation.

See the [TypeScript SDK clients docs](/services/invocation/clients/typescript-sdk#custom-serialization) for details.
