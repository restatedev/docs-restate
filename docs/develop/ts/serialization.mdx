...
When invoking Restate handlers from outside a Restate handler (using `@restatedev/restate-sdk-clients`), you can set a default serde for all calls made via that client by passing a `serde` option to `clients.connect()`.
This saves you from repeating the serde on every individual call when your services use a non-JSON format by default.
...
The default serde applies to all handler calls, workflow attach/output polling, awakeable resolution, and attached invocation results.
Individual calls can still override the default by passing per-call options:

```typescript
clients.rpc.opts({ input: restate.serde.json, output: restate.serde.json })
```

If no default is set, the ingress client falls back to `restate.serde.json`.
