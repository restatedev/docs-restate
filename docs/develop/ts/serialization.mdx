...
## Setting `Content-Type` on empty responses

By default, when a handler returns an empty body (zero bytes after serialization), Restate does not include a `Content-Type` header in the response.

For some serialization formats — such as [Protobuf](https://protobuf.dev/), where an empty message serializes to zero bytes — the `Content-Type` header must be present even for empty responses so callers can deserialize correctly.

Set `setOutputContentTypeIfEmpty: true` in the handler options to force the `Content-Type` header to be sent regardless of body size:

```typescript {"CODE_LOAD::ts/src/develop/serialization.ts#content_type_if_empty"}
