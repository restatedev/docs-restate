...
When sending a request to a handler configured with custom serde(s) you need to specify them on the client.
You can set a default serde for all calls on the [ingress client connection](/services/invocation/clients/typescript-sdk#custom-serialization), or specify them per call.
To customize the serde to use on in-context service calls:
