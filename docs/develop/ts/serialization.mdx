...
## Default serde for ingress clients

When using the [`@restatedev/restate-sdk-clients`](https://www.npmjs.com/package/@restatedev/restate-sdk-clients) package to call Restate handlers from outside a handler, you can set a connection-wide default serde:
...
This default applies to all calls made through this client, including handler calls, workflow operations, awakeable resolution, and result retrieval. You can still override it per call using `rpc.opts` or `rpc.sendOpts`. For more details, see the [TypeScript SDK clients documentation](/services/invocation/clients/typescript-sdk#custom-serialization).
