...
When using the [TypeScript SDK ingress client](/services/invocation/clients/typescript-sdk) to call services from outside a Restate handler, you can set a default serde on the connection. This applies to all calls, workflow attach/output, awakeable resolution, and result polling — without repeating the serde on every call.

See [Custom serialization in the SDK clients docs](/services/invocation/clients/typescript-sdk#custom-serialization) for details.
