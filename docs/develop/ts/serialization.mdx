...
When using the ingress client (`@restatedev/restate-sdk-clients`), you can set a default serde for all operations by passing it in the connection options.
This is useful when your services use a non-JSON encoding (such as Protobuf, MessagePack, or a custom binary format) and you want to avoid specifying the serde on every individual call.
...
// Per-call serde still overrides the default when needed
...
- Invocation result retrieval via `restateClient.result(...)`

Per-call serde options always take precedence over the connection-level default.
If no default is configured and no per-call serde is given, the client falls back to `restate.serde.json`.
