...
When using the [ingress client](/services/invocation/clients/typescript-sdk) from outside a Restate handler, you can set a default serde on the connection that applies to all operations:
...
Per-operation overrides take precedence over the connection-level default.
