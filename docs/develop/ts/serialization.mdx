...
When using `@restatedev/restate-sdk-clients` to invoke handlers from outside a Restate service, you can set a default serde on the connection. This is applied to all calls made through the client unless overridden per operation.
...
- Workflow `attach` and `output` polling results
- Awakeable resolution payloads
- Invocation results retrieved via `restateClient.result(...)`

You can still override the serde for a specific operation using per-call options:
