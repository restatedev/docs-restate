...
## Ingress client serialization

When invoking handlers from outside Restate using the [TypeScript SDK client library](/services/invocation/clients/typescript-sdk), you can set a default serde on the connection so it applies to all operations without repeating it on every call:
...
Per-call options take precedence over the connection-level default. See the [SDK clients docs](/services/invocation/clients/typescript-sdk#custom-serialization) for details and per-call override examples.
