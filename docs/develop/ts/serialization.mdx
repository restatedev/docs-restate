...
When using the [ingress client](/services/invocation/clients/typescript-sdk) from outside a Restate handler, you can set a default serde on the connection so it applies to all calls:
...
const restateClient = clients.connect({
  ...
  serde: restate.serde.binary, // applied to all calls unless overridden
});
```

See the [ingress client docs](/services/invocation/clients/typescript-sdk#custom-serialization) for more details.
