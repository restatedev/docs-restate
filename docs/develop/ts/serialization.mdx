...
When sending a request to a handler configured with custom serde(s) you can specify them at the per-call level, or configure a default serde on the client.
To customize the serde to use on requests from inside a handler:
...
## Ingress client default serialization

When using the [ingress client](/services/invocation/clients/typescript-sdk) to call services from outside Restate, you can set a default `serde` on the connection options.
This applies to all operations (handler calls, workflow attach/output, awakeable resolution, and result retrieval) unless overridden per-call:
...
The precedence order is: **per-call serde > connection default serde > built-in JSON**.

See [ingress client docs](/services/invocation/clients/typescript-sdk#custom-serialization) for more details.
