...
#### Setting a default serde on the ingress client

When using the [ingress client](/services/invocation/clients/typescript-sdk) (`@restatedev/restate-sdk-clients`), you can set a connection-level default serde so all calls automatically use the correct format without specifying it per operation:
...
Per-operation serde options take precedence when specified.
