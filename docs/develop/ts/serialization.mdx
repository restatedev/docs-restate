...
When using the external ingress client (`@restatedev/restate-sdk-clients`), you can set a connection-level default serde instead of specifying it on every call. See [TypeScript SDK clients](/services/invocation/clients/typescript-sdk#customizing-serialization) for details.
