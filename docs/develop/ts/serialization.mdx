...
By default, the TypeScript SDK uses the built-in [JSON](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON) support to perform (de)serialization.

You can configure a default `serde` at the service or endpoint level, and then override it per handler as needed:

```typescript {"CODE_LOAD::ts/src/develop/serialization.ts#default_serde"} 
