...
When using `@restatedev/restate-sdk-clients` to call Restate handlers from outside a Restate context, you can set a default `serde` on the client connection. This serde is used for all operations — handler calls, workflow attaches and output polling, awakeable resolution, and attached invocation results — unless overridden per call.
...
This is especially useful when all or most of your handlers use a non-JSON serde, so you don't have to repeat the serde option on every call.

<Info>
See the [TypeScript SDK Clients docs](/services/invocation/clients/typescript-sdk) for more details on the ingress client.
</Info>
