...
When using the [ingress client](/services/invocation/clients/typescript-sdk), you can set a default serde on the connection to avoid repeating it on every call:
...
const restateClient = clients.connect({
  ...
  serde: restate.serde.binary, // default for all operations on this client
});
```

The per-call serde option takes precedence over the connection-level default, which in turn takes precedence over `restate.serde.json`.
