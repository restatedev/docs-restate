...
When invoking handlers from outside Restate using the [TypeScript SDK client library](/services/invocation/clients/typescript-sdk), you can set a default serde on the connection. This applies to all handler invocations, workflow attaches, awakeable resolution, and result retrieval — without having to specify the serde on each individual call.
...
Individual calls can still override the connection-level default using per-call options. See the [SDK clients docs](/services/invocation/clients/typescript-sdk#custom-serialization) for details.
