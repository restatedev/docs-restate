...
When using the [SDK ingress client](/services/invocation/clients/typescript-sdk) to call services from outside a handler, you can set a default serde on the client connection instead of specifying it per-call.
...
The default serde applies to all operations on that client unless overridden per-call using the `opts` parameter.
