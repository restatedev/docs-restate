...
## Tunnel deployment (preview)

<Note title="Preview feature">
`@restatedev/restate-sdk-tunnel` is a preview package. Its API may change in future releases.
</Note>

`@restatedev/restate-sdk-tunnel` is a drop-in alternative to `restate.serve(...)` for services running in private networks. Instead of binding an inbound HTTP listener, it holds an **outbound** connection to Restate Cloud — no public IP or inbound networking required.
...
Then replace `restate.serve(...)` with `connectTunnel`:
...
const connection = connectTunnel({
  ...
  authToken: process.env.RESTATE_AUTH_TOKEN!,   // Cloud API key with Full role
...
See the [package README](https://github.com/restatedev/sdk-typescript/tree/main/packages/libs/restate-sdk-tunnel) for Kubernetes setup and registration details.
