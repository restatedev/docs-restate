...
<Accordion title="Customizing the HTTP server">

If you need to manually control or customize the HTTP server, use `restate.createEndpointHandler` to create a handler and then use it to manually instantiate the HTTP server.

The handler auto-detects the HTTP version per request, so it works with both HTTP/1.1 and HTTP/2:
