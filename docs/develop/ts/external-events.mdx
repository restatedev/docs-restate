...
You can also pass a `TerminalError` to propagate a specific error code, message, and metadata to the waiting handler:

```ts {"CODE_LOAD::../snippets/ts/src/develop/awakeable.ts#reject_terminal_error"} 
