...
An awakeable is a special type of signal, specialized for one-shot resolution with a generated unique ID. Use it when an external system needs a task token that it can resolve or reject through Restate.
