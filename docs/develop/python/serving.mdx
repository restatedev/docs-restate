...
Then, [register the service](/operate/registration#registering-a-deployment) with Restate specifying this route, here `localhost:9080/restate/v1`.
</Accordion>
...
Uvicorn does not support HTTP/2, so you need to tell Restate to use HTTP/1.1 when [registering the deployment](/operate/registration#deployments-supporting-only-http1-1).
