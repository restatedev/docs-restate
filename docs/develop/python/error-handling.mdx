...
You can attach structured metadata to a `TerminalError` using the optional `metadata` dictionary. The metadata is propagated alongside the error message and status code, and is accessible to callers:

```python
raise TerminalError(
    "Payment declined",
    status_code=402,
    metadata={"reason": "insufficient_funds", "retry_after": "2026-07-21"},
)
```

Use metadata to attach machine-readable context to terminal failures, such as error codes, correlation IDs, or structured details for the caller to act on.
...
```python {"CODE_LOAD::python/src/develop/error_handling.py#retryable"} 
...
```python {"CODE_LOAD::python/src/develop/error_handling.py#retryable_run"} 
from datetime import timedelta
from restate.exceptions import RetryableError
