...
<Accordion title="How is data backed up?">
  Configurations that span multiple availability zones use synchronous replication across nodes, periodic persistent volume snapshots, and periodic snapshots to cloud object storage. Single node configurations rely on volume snapshots to cloud object storage.
</Accordion>
