...
Use `clients.connect()` to create an ingress client. You can optionally pass a `serde` option to configure a default serializer/deserializer for all operations. When provided, this serde is used for handler calls, workflow submit/attach/output, awakeable resolution, and attached invocation results. It falls back to `serde.json` if omitted.

```ts
import * as clients from "@restatedev/restate-sdk-clients";

// Default connection — uses JSON serde
const restateClient = clients.connect({ url: "http://localhost:8080" });

// With a custom default serde applied to all operations
const restateClient2 = clients.connect({
...
```ts
// This overrides the connection-level default for this specific call
await restateClient2
  .serviceClient(myService)
  .myHandler(input, clients.rpc.opts({ input: otherSerde, output: otherSerde }));
```
...
- **[Serialization](/develop/ts/serialization)**: Customize how data is serialized for handler calls
- **[External events (awakeables)](/develop/ts/external-events)**: Signal running invocations from external systems
