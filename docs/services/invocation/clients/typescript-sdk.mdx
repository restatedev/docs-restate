...
By default, the ingress client uses JSON serialization for all requests and responses.
If your services use [custom serialization](/develop/ts/serialization#custom-serialization), you can configure a default `serde` on the connection options.
This default applies to all ingress operations — handler calls, workflow attach/output, awakeable resolution, and result retrieval — unless you override it at the per-call level.
...
const restateClient = clients.connect({
  ...
  serde: myCustomSerde, // used for all calls unless overridden
});
```

To override the serde for a specific call, pass it through the call options:
...
await restateClient
  ...
  .greet(
    { greeting: "Hi" },
    clients.rpc.opts({ input: myInputSerde, output: myOutputSerde })
  );
```

For awakeable resolution and result retrieval, pass the serde directly:

```typescript
// Override serde when resolving an awakeable
await restateClient.resolveAwakeable(id, payload, myPayloadSerde);

// Override serde when retrieving a result
const result = await restateClient.result(handle, myResultSerde);
```

<Info>
    The per-call serde always takes precedence over the connection-level default.
    The connection-level default takes precedence over the built-in JSON fallback.
</Info>
