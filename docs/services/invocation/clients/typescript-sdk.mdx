...
By default, the ingress client uses JSON for all payloads. You can set a different default serde when connecting:
...
- Invocation result retrieval

You can still override serde on a per-call basis using the `input`/`output` options on individual calls. Per-call options take precedence over the connection default.

<Info>
  Use a connection-level default serde when your Restate services use a non-JSON encoding (such as Protobuf or a custom binary format) across the board. This saves you from specifying the serde on every individual call.
</Info>
