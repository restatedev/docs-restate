...
By default, the ingress client serializes request and response payloads as JSON. If your handlers use a different serialization format, you can set a default serde on the client connection. This applies to all operations: handler calls, workflow attach/output polling, awakeable resolution, and result retrieval.
...
Per-operation serde options (passed via `clients.rpc.opts()` or `clients.rpc.sendOpts()`) take precedence over the connection-level default. See the [serialization docs](/develop/ts/serialization) for available serde implementations.
