...
By default, the client uses JSON for all payloads. If your handlers use a custom serde, you can set a default serde at connection time so you don't have to specify it on every call:
...
Per-call serde options still override the connection default. See the [serialization docs](/develop/ts/serialization#ingress-client-default-serde) for full details.
