...
By default the ingress client uses JSON for all handler calls. If your services use a different format (for example binary or Protobuf), you can set a default serde once when creating the client instead of specifying it on every call:
...
The default applies to all calls made via that client instance (handler calls, workflow attach/output polling, awakeable resolution, and result retrieval).
You can still override the serde for a specific call by passing per-call options:

```typescript
clients.rpc.opts({ input: restate.serde.json, output: restate.serde.json })
```

See the [serialization docs](/develop/ts/serialization) for the full list of built-in serde options and how to implement your own.
