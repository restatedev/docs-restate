...
By default, the ingress client uses JSON for all requests and responses. If your handlers use custom serialization (see [Serialization](/develop/ts/serialization)), you can configure a default serde for all calls on the connection:
...
This default serde applies to all operations: handler calls, workflow attach/output polling, awakeable resolution, and `.result()` calls.

You can still override it per-call using `clients.rpc.opts(...)`:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#ingress_serde_override"} 
// Override the connection-level default for this specific call
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(
    input,
