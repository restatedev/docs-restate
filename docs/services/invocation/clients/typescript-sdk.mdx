...
By default, the ingress client serializes and deserializes payloads as JSON. If your handlers use a custom `Serde`, you need to tell the client which serializer to use.

### Per-call serde

You can specify a custom serde for individual calls using the call options:
...
// Call a handler that uses binary serde
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(
    inputPayload,
...
### Connection-level default serde

If all (or most) of your handlers use the same custom serde, you can set a default serde when connecting. This applies to all operations — handler calls, workflow attach/output polling, awakeable resolution, and attached invocation results — unless overridden at the call level.
...
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(inputPayload);

// You can still override per-call when needed
const otherResult = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myJsonHandler(
    jsonPayload,
...
The serde precedence is: **per-call serde → connection default serde → `restate.serde.json`**.

<Tip>
  The connection-level `serde` option is useful when your services use a custom serialization format (e.g., binary, Protobuf, or a prefixed encoding) and you want to avoid specifying the serde on every call.
</Tip>
