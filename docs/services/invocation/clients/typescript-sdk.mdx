...
`clients.connect()` accepts a `ConnectionOpts` object with the following fields:

| Field | Description |
|-------|-------------|
| `url` | The URL of the Restate ingress endpoint. |
| `headers` | Optional HTTP headers to include with every request. |
| `serde` | Default serializer/deserializer for all operations on this client. Defaults to `restate.serde.json`. |

If your services use a non-JSON encoding, you can set a default `serde` once at connection time instead of specifying it on every call. See [Serialization](/develop/ts/serialization#ingress-client-default-serde) for details.
