...
By default, the ingress client uses JSON for all payloads. If your services use a different encoding, you can set a default serde for all operations on a connection by passing `serde` to `clients.connect()`:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress_serde.ts#connect_serde"}
```

The `serde` option applies to:
- Handler invocations (request-response and one-way)
- Workflow attach and output polling
- Awakeable resolution (`resolveAwakeable`)
- Invocation result retrieval (`result`)

Per-call overrides always take precedence. Use the existing per-operation serde parameters for individual calls:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress_serde.ts#per_call_override"}
```

Since v1.14.1, `resolveAwakeable` and `result` also accept an explicit serde parameter:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress_serde.ts#awakeable_serde"}
```

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress_serde.ts#result_serde"}
```

For more on defining custom `Serde` implementations, see the [TypeScript serialization docs](/develop/ts/serialization).
