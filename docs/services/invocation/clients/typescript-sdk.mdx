...
By default, the ingress client uses JSON serialization for all requests and responses. If your handlers use a [custom serde](/develop/ts/serialization), you can configure it at the connection level so you don't have to repeat it on every call.
...
const restateClient = clients.connect({
  ...
  serde: restate.serde.binary, // used for all calls unless overridden per-call
});
```

This is useful when you have services that consistently use a non-JSON encoding (for example, Protobuf or a custom binary format) and you don't want to specify the serde on every individual call.

### Overriding serde per call

You can still override the serde on individual calls. The per-call serde takes precedence over the connection-level default:
...
The `serde` option also applies when resolving awakeables and retrieving invocation results. You can override it per-call using the optional `payloadSerde` and `resultSerde` parameters:

```typescript
// Resolve an awakeable — uses connection-level serde by default
await restateClient.resolveAwakeable(id, payload);

// Resolve an awakeable with a per-call serde override
...
const result2 = await restateClient.result(handle, restate.serde.binary);
```
