...
By default, the ingress client uses JSON serialization for all payloads. If your handlers use a custom serde (for example, `restate.serde.binary` or a schema-validated serde), you can configure a default serde at the connection level so you don't have to specify it on every call.

Set the `serde` option when calling `clients.connect()`:
...
- Result retrieval via `restateClient.result()`

You can still override the serde for any individual call using per-call options (`clients.rpc.opts({ input: ..., output: ... })`). Per-call options take precedence over the connection default.

See [serialization](/develop/ts/serialization) for the full list of available serdes and how to implement a custom one.
