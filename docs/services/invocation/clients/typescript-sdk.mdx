...
By default, the ingress client uses JSON to serialize and deserialize payloads. If your handlers use a [custom serde](/develop/ts/serialization), you can set a default serde on the connection so you don't need to specify it on every call.
...
This is useful when your services consistently use a non-JSON encoding and you don't want to specify the serde on every individual call.
...
The connection-level `serde` also applies when resolving awakeables and retrieving invocation results. You can override it per call with the `payloadSerde` parameter on `resolveAwakeable` and the `resultSerde` parameter on `result`:

```typescript
// Resolve an awakeable — uses connection-level serde by default
await restateClient.resolveAwakeable(id, payload);

// Override the serde for this awakeable resolution
...
// Override the serde for this result retrieval
