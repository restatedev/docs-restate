...
By default, the client uses JSON serialization for all requests and responses. You can configure a different default serde when connecting, which applies to all handler calls, workflow attach/output polling, awakeable resolution, and attached invocation results.
...
Any `serde` specified in `rpc.opts()` or `rpc.sendOpts()` for an individual call overrides the default.

For more details, see [Serialization](/develop/ts/serialization).
