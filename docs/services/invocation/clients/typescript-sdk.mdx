...
By default, the ingress client uses JSON to serialize and deserialize payloads. If your handlers use a [custom serde](/develop/ts/serialization), you can set a default serde on the connection so you don't have to repeat it on every call.

### Setting a default serde

Pass a `serde` option to `clients.connect()` to use it as the default for all handler calls, workflow attaches and output polling, awakeable resolution, and attached invocation results:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#ingress_default_serde"}
// import * as clients from "@restatedev/restate-sdk-clients";
...
This is useful when your services consistently use a non-JSON encoding (for example, Protobuf or a custom binary format) and you don't want to specify the serde on every individual call.

### Overriding serde per call

Per-call options take precedence over the connection-level default:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#ingress_per_call_serde"}
...
### Serde for awakeables and results

The connection-level `serde` also applies when resolving awakeables and retrieving invocation results. You can override it per call using the optional `payloadSerde` parameter on `resolveAwakeable` and `resultSerde` parameter on `result`:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#ingress_awakeable_serde"}
...
<Info>
The serde priority order is: per-call serde → connection-level default serde → `restate.serde.json`.
</Info>
