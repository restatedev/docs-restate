...
By default, the ingress client uses JSON serialization for all payloads. If your handlers use [custom serde](/develop/ts/serialization), you can configure a default serde on the connection, or specify it per-call.

### Default serde for all calls

Set a `serde` on `clients.connect()` to use it as the default for all operations — handler calls, workflow attach/output polling, awakeable resolution, and result retrieval:
...
const restateClient = clients.connect({
  ...
  serde: myCustomSerde, // used as default for all calls
});

// All calls now use myCustomSerde unless overridden
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(input);
```

The precedence order is: per-call serde option > connection-level default serde > `restate.serde.json`.

### Per-call serde override

You can override the serde for individual calls using `clients.rpc.opts()`:

```typescript
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(input, clients.rpc.opts({ input: myInputSerde, output: myOutputSerde }));
```

### Awakeable resolution and result retrieval

`resolveAwakeable` and `result` also accept an optional serde parameter:

```typescript
// Resolve an awakeable with custom serde
await restateClient.resolveAwakeable(id, payload, myPayloadSerde);

// Retrieve an invocation result with custom serde
const output = await restateClient.result(handle, myResultSerde);
```

For more details on implementing a custom `Serde`, see the [serialization docs](/develop/ts/serialization).
