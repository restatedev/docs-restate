...
By default, the ingress client serializes and deserializes payloads using JSON. If your service handlers use a custom `Serde`, you can set a default serde once at connection time so you don't have to pass it on every call:
...
- Handler calls (input and output)
- Workflow `attach` and `output` polling
- Awakeable resolution payloads
- Attached invocation results via `result()`

Per-call options (passed as the last argument to each method) take precedence over the connection-level default.
