...
By default, the client uses JSON serialization. If your handlers use custom serialization, you can set a default serde on the connection so you don't need to specify it on every call:
...
The `serde` connection option applies to all handler call inputs and outputs, workflow attach/output results, awakeable resolutions, and results retrieved via `restateClient.result(...)`. You can still override it per-call using `clients.rpc.opts({ input: ..., output: ... })`.

For more details on serialization, see the [Serialization docs](/develop/ts/serialization).
