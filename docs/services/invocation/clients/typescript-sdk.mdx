...
By default, the ingress client serializes all payloads as JSON. You can set a different default serde when connecting, so it applies to every operation on that client — handler calls, workflow attaches, awakeable resolution, and attached invocation results.

```ts {"CODE_LOAD::ts/src/develop/clients/ingress.ts#default_serde"}
```

Per-operation serde options still take precedence over the connection-level default.
