...
By default, the ingress client uses JSON serialization. You can override this with a custom default serde when creating the connection:
...
The default serde applies to all handler calls, workflow attach/output polling, awakeable resolution, and result retrieval made through that client. You can still override it per-call with the `opts` parameter.

See [serialization](/develop/ts/serialization#default-serde-for-ingress-clients) for details on available serde options.
