...
By default, the ingress client uses JSON to serialize and deserialize all payloads. If your handlers use [custom serialization](/develop/ts/serialization), you can configure a default serde for the connection so that all calls automatically use the correct format.

Pass the `serde` option to `clients.connect()` to set a connection-level default:
...
const restateClient = clients.connect({
  ...
  serde: restate.serde.binary,   // used for all calls unless overridden
});
```

The `serde` option applies to:
- Handler input and output (request-response and one-way calls)
- Workflow attach and output polling
- Awakeable resolution payload
- Invocation result retrieval via `restateClient.result()`

Per-operation serde options still take precedence over the connection default when specified.
