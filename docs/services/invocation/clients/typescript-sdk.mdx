...
By default, the ingress client uses JSON serialization for all payloads. If your handlers use a non-JSON serde (such as binary or a custom format), you can set a default `serde` when connecting. This applies to all handler calls, workflow attaches/output polling, awakeable resolution, and attached invocation results.
...
You can still override the serde on individual calls by passing `input`/`output` options via `clients.rpc.opts(...)`.

<Info>
See the [serialization docs](/develop/ts/serialization) for details on built-in serdes and how to implement custom ones.
</Info>
