...
By default, the ingress client uses JSON to serialize and deserialize payloads. If your handlers use a [custom serde](/develop/ts/serialization), you can configure it on the client.

### Setting a default serde

You can set a default serde in `ConnectionOpts` when calling `clients.connect()`. This serde is used as the fallback for all operations on that client — handler calls, workflow attach/output polling, awakeable resolution, and invocation result retrieval — unless a per-operation serde overrides it.
...
1. Per-operation serde (passed in call options)
2. Connection-level default serde (set in `ConnectionOpts`)
3. `restate.serde.json` (built-in default)

For more details on implementing custom serializers, see the [Serialization docs](/develop/ts/serialization).
