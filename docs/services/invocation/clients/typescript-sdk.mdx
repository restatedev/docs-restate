...
## Custom serialization (default serde)

When your services use a custom serialization format, you can set a default `serde` on the connection instead of specifying it on every individual call.

```typescript
import * as clients from "@restatedev/restate-sdk-clients";

// A custom serde — for example, MessagePack or a protobuf codec.
const myCustomSerde = /* ... your Serde<any> implementation ... */;
...
- Handler call inputs and outputs
- Workflow attach and output polling
- Awakeable resolution payloads
- Attached invocation results

If you supply a per-call serde (via `opts.input` / `opts.output` or `payloadSerde`), it takes precedence over the connection default. If neither is set, JSON is used.
