...
By default, the ingress client uses JSON serialization. You can override this for all calls made through a connection by passing a `serde` option to `clients.connect()`:
...
const restateClient = clients.connect({
  ...
  serde: clients.serde.binary, // applies to all calls unless overridden per-call
});
```

This default applies to handler calls, workflow attaches, output polling, awakeable resolution, and attached invocation results. You can still override the serde on individual calls using `clients.rpc.opts({ input: ..., output: ... })`.

For more details on available serde options, see the [serialization docs](/develop/ts/serialization).
