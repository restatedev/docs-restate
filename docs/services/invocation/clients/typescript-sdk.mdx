...
By default, the ingress client serializes all payloads as JSON. If your handlers use a custom serde (for example, Protobuf or a prefixed binary format), you can set a connection-wide default when calling `clients.connect()`:
...
- All handler calls (service, object, and workflow)
- Workflow `workflowSubmit`, `workflowAttach`, and `workflowOutput`
- `resolveAwakeable` payloads
- `result()` deserialization for attached invocations

To override the default for a specific call, pass serde options directly on that call:

```typescript
import * as clients from "@restatedev/restate-sdk-clients";

// Override input and output serde for a single call
await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .greet(
    { greeting: "Hi" },
    clients.rpc.opts({ input: myCustomSerde, output: myCustomSerde })
  );
...
const response = await restateClient.result(handle, myCustomSerde);
```

Per-call serde options always take precedence over the connection default.
