...
By default, the ingress client uses JSON serialization for all payloads. If your handlers use a [custom `Serde`](/develop/ts/serialization#custom-serialization), you can configure a default serde on the client connection so you don't have to specify it on every individual call.

### Default serde for all calls

Pass a `serde` option to `clients.connect()` to set a default serializer/deserializer for all calls made through that client:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#default_serde"}
import * as clients from "@restatedev/restate-sdk-clients";
import * as restate from "@restatedev/restate-sdk";

// Create a client with a custom default serde
const restateClient = clients.connect({
  url: "http://localhost:8080",
  serde: myCustomSerde,
});

// All calls use myCustomSerde for input/output serialization by default
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(myInput);
```

The default serde applies to:
- Handler calls (request-response and one-way)
- Workflow attach and output polling
- Awakeable resolution (`resolveAwakeable`)
- Attached invocation results (`result`)

### Per-call serde override

You can still override the serde on individual calls using the per-call options. The per-call serde takes precedence over the connection default:

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#per_call_serde_override"}
import * as clients from "@restatedev/restate-sdk-clients";

const restateClient = clients.connect({
  url: "http://localhost:8080",
  serde: myDefaultSerde, // used unless overridden
});

// This call uses the per-call serde instead of the default
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(
    myInput,
...
For more details on implementing custom serdes, see the [serialization docs](/develop/ts/serialization).
