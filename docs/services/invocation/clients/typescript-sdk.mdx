...
By default, the ingress client serializes payloads using JSON. If your services use a different encoding (for example, a custom binary format), you can set a default serde for all operations on a connection by passing `serde` to `clients.connect()`:
...
const rs = clients.connect({
  ...
  serde: myCustomSerde, // applied to all calls on this connection
});
```
...
Per-call overrides always take precedence. You can override the serde for individual calls using the existing per-operation serde parameters, and you can now also pass a `payloadSerde` to `resolveAwakeable` and a `resultSerde` to `result`:
...
For more information on defining custom `Serde` implementations, see the [TypeScript serialization docs](/develop/ts/serialization).
