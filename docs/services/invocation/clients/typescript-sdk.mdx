...
By default, the ingress client uses JSON (`restate.serde.json`) to serialize and deserialize all payloads. If you use a [custom serde](/develop/ts/serialization) on your handlers, you can set a matching default at the client level so you don't have to repeat the serde option on every call.

Pass a `serde` option to `clients.connect()`:

```typescript
import * as clients from "@restatedev/restate-sdk-clients";
import * as restate from "@restatedev/restate-sdk";

// A custom serde — any object implementing the Serde interface works
const mySerde = restate.serde.binary; // example: binary serde
...
// All calls now use mySerde by default:
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(input);
```
...
- Handler calls (request and response bodies)
- Workflow `attach` and `output` polling
- Awakeable resolution payloads
- `result()` calls on send handles

You can still override the serde for individual calls using the existing `opts.input` / `opts.output` per-call options:

```typescript
// This call uses an operation-specific serde, ignoring the client default
const result = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .myHandler(
    input,
    clients.rpc.opts({ input: overrideSerde, output: overrideSerde })
  );
```
