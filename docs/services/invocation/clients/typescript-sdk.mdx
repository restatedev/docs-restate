...
By default, the ingress client uses JSON for all payloads. If your services use a different encoding, you can set a default serde for an entire connection by passing `serde` to `clients.connect()`:
...
`resolveAwakeable` and `result` also accept an explicit serde parameter for per-call overrides:
