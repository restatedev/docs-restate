...
By default, the client uses JSON serialization for all payloads. You can set a custom default serializer at connection time, which applies to all handler calls, workflow operations, awakeable resolution, and invocation result retrieval on that client:
...
You can override the default for individual calls by passing per-operation serde options, as described in the [serialization docs](/develop/ts/serialization).
