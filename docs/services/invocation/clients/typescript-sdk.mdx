...
Pass your Restate ingress URL to `clients.connect()`. You can also set connection-wide defaults here:
...
const restateClient = clients.connect({
  ...
  // Optional: attach authentication or custom headers to every request
  headers: { Authorization: "Bearer my-token" },
  // Optional: default serializer/deserializer for all operations.
  // Applies to handler calls, workflow attach/output, awakeable resolution,
  // and attached invocation results. Defaults to JSON. Can be overridden
  // per-call via Opts or SendOpts.
  serde: clients.serde.binary,
});
```

The `serde` option is useful when your services use a custom wire format (e.g. Protobuf or MessagePack) — set it once at connection time instead of repeating it at every call site. Individual calls can still override it via the `input`/`output` fields in `Opts`.
