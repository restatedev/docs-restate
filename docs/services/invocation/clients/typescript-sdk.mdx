...
By default, the ingress client uses JSON serialization for all payloads. If your services use [custom serialization](/develop/ts/serialization), you can set a default serde on the client connection so you don't have to repeat it on every call:
...
The `serde` option in `ConnectionOpts` applies as the fallback for all operations: handler calls, workflow attach/output polling, awakeable resolution, and attached invocation results. Individual calls can still override this by passing per-call serde options.

The priority order is:
1. Per-call serde option (e.g., `clients.rpc.opts({ input: ..., output: ... })`)
2. Connection-level default serde (`serde` in `ConnectionOpts`)
3. `restate.serde.json` (built-in fallback)

<Info>
This is useful when all or most of your services use the same custom serde, such as binary, Protobuf, or a custom schema. Set it once on `connect()` and override only where needed.
</Info>
