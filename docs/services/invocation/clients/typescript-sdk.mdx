...
By default, the ingress client uses JSON serialization for all payloads. If your services use a [custom serde](/develop/ts/serialization), you can set a default serde on the connection that applies to all calls made through that client — handler invocations, workflow attach/output, awakeable resolution, and result retrieval.

```typescript {"CODE_LOAD::ts/src/develop/clients/ingress.ts#default_serde"}
```

You can still override the serde for individual calls by passing per-call options.
