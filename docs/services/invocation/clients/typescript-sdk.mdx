...
When all your services use a custom serialization format, you can set a default `serde` on the connection instead of specifying it per call. The connection-level serde applies to all handler calls, workflow attach/output polling, awakeable resolution, and attached invocation results.

```typescript
import * as clients from "@restatedev/restate-sdk-clients";
import { mySerde } from "./my-serde";
...
Per-call `input`/`output` options still override the connection-level default:

```typescript
// Override serde for this specific call only
const greet = await restateClient
  .serviceClient<MyService>({ name: "MyService" })
  .greet({ greeting: "Hi" }, clients.rpc.opts({ input: otherSerde, output: otherSerde }));
```

See the [serialization docs](/develop/ts/serialization) for details on implementing a custom `Serde`.
