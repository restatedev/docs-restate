...
By default, the ingress client uses JSON serialization for all requests and responses. You can set a different default when creating the connection — it applies to all handler calls, workflow attach and output polling, awakeable resolution, and invocation result retrieval.
...
Per-call options (`rpc.opts({ input, output })` / `rpc.sendOpts`) take precedence over the connection default. The `resolveAwakeable` and `result` methods also accept an explicit serde as a last parameter to override the default for that single call.

```ts {"CODE_LOAD::ts/src/develop/clients/ingress.ts#default_serde_awakeable"}
...
For more details on custom serialization, see the [serialization docs](/develop/ts/serialization).
