...
<Warning>
    This page describes the old Kafka support that runs the consumer inside the Restate server.
    For new projects, use the standalone [Kafka ingress integration](/services/integrations/kafka) instead, which scales independently and provides support for the larger Kafka ecosystem.
    If you are already using the old support, follow the [migration guide](/services/integrations/kafka#migrating-from-the-old-kafka-support).
</Warning>
