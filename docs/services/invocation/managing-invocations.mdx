...
<Accordion title="How Cancellation Works">
    ...
    <CodeGroup>
    ```typescript {"CODE_LOAD::code/ts/src/cancellation-example.ts"}
    ```
    ```go {"CODE_LOAD::code/go/cancellation-example.go"}
    ```
    ```java {"CODE_LOAD::code/java/src/main/java/cancellation/CancellationExample.java"}
    ```
    ```python {"CODE_LOAD::code/python/cancellation_example.py"}
    ```
    </CodeGroup>
