...
You can configure a global default retry policy for all invocations in your cluster as follows:
...
This will retry the invocation 100 times, after which the invocation will be _paused_ if no progress can be made. To resume a paused invocation, check the paragraph below.
