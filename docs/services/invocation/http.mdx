...
<Note title="Restate <= 1.6">
    Use the path `/{service}/{handler}` or `/{service}/{key}/{handler}` instead.
</Note>
...
<Note title="Restate <= 1.6">
    Use the path `/{service}/{handler}/send` or `/{service}/{key}/{handler}/send` instead.
</Note>
