...
In long-running applications, your business logic might span multiple code versions: step 1 runs on v1, you ship v2, and now step 2 must understand v1's output.
Every step becomes a version compatibility challenge that you need to face.

Restate helps you with the concept of **immutable deployments**: an entire invocation gets pinned to a specific version of your code, so you never deal with mid-execution version mismatches.
Deploy as often as you want: in-flight requests complete on their original version, new requests go to the latest.

## What is a deployment?

A deployment is a specific instance of your service code running as an HTTP endpoint, Lambda function, or other supported environment.
New requests automatically route to the latest deployment, while in-flight requests complete on their original one.

Deployments can be managed through the [Restate UI](/installation#restate-ui), the CLI, or the [Admin API](/admin-api/deployment):
...
<Note>
    In-flight invocations might fail with [non-determinism errors](/references/errors#rt0016), but this is typically fine during development.
</Note>
...
This section covers common scenarios you may encounter when managing deployments.
...
For example, given you register a new revision for service `Greeter`, renaming its handler from `greet` to `sayGreet` is a breaking change, thus registration will fail.
...
If you have a bug in your deployed code, you can fix it by updating the code at the same endpoint.
This is safe as long as your fix doesn't change the deterministic behavior of already-executed journal entries.
...
The fix MUST be determinism-compatible with the already-executed journal entries.
...
### Advanced: Reassigning a deployment endpoint
