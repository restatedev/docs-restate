...
_Invocation_s have a unique **Invocation ID** starting with `inv_`. You can find this ID in every place where an invocation is mentioned:
...
All Invocations follow a well-defined lifecycle:
...
Completed: Invocation completed, retained for observability
...
Administrators/Operators can interact with the invocation lifecycle through the UI, CLI, or manually calling the REST Admin API.
...
You can also manually purge invocations if you need to free up disk space:
