...
<Accordion title="How does the standalone tunnel client work?">
    A standalone tunnel client maintains the outbound connection to Restate Cloud and forwards invocations to your service over its normal HTTP endpoint. You can run it beside the service or elsewhere in the same private network, and run multiple tunnel clients for redundancy.

    <Frame>
        <img src="/img/cloud/tunnel_deployment.png" alt="Restate Cloud or BYOC connecting to a private service through a standalone tunnel client" />
    </Frame>
</Accordion>
</AccordionGroup>
...
```
        ...
        - `RESTATE_BEARER_TOKEN`: Create an API key with the **Full** role under [**Developers > API Keys**](https://cloud.restate.dev/to/developers/integration?createApiKey=true&createApiKeyDescription=tunnel-key&createApiKeyRole=rst:role::FullAccess). It starts with `key_`.
