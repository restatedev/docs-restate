...
<Note>
  Native Google OIDC authentication to private Cloud Run services requires a Google-recognized identity for `restate-server`: either an ambient GCP workload identity, or an AWS identity federated into your Google Cloud project via a workload identity provider.
  On [Restate Cloud](/cloud/getting-started), use the latter — see [connecting Google Cloud Run services](/cloud/connecting-services#google-cloud-run-services) for the environment-specific trust setup, then register with the flags described on this page.
</Note>
...
A Restate server whose ambient identity is an AWS identity (such as Restate Cloud, or a self-hosted deployment on EKS/EC2) can mint Google ID tokens without any Google credential, by exchanging a signed AWS identity assertion through a [workload identity federation](https://cloud.google.com/iam/docs/workload-identity-federation) provider in your Google Cloud project. Register with the provider's full resource name, always paired with a service account to impersonate:

```shell
restate deployments register https://svc-abc-uc.a.run.app \
  --gcp-workload-identity-provider="//iam.googleapis.com/projects/<PROJECT_NUMBER>/locations/global/workloadIdentityPools/<POOL>/providers/<PROVIDER>" \
  --gcp-impersonate-service-account caller@my-proj.iam.gserviceaccount.com
```

The server operator configures which AWS role is assumed for federation (`worker.invoker.gcp-federation` in the server configuration); the provider you create must trust that role, and the impersonation grant is made to the resulting federated subject. On Restate Cloud this is preconfigured — follow [connecting Google Cloud Run services](/cloud/connecting-services#google-cloud-run-services) for the exact trust setup.
