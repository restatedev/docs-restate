...
icon: "/img/cloud-providers/google-cloud-run.svg"
---
...
<Note>
  Native Google OIDC authentication to private Cloud Run services is only available when `restate-server` runs with a GCP workload identity. It is **not currently supported on [Restate Cloud](/cloud/getting-started)** — registering a private Cloud Run deployment against a Restate Cloud environment will fail discovery with `403 Forbidden`. Support in Restate Cloud is on the roadmap; in the meantime, deploy the service as a public Cloud Run service (`--allow-unauthenticated`) and secure it with [Restate request signing](/services/security#locking-down-service-access) so the SDK authenticates the calling environment.
</Note>
