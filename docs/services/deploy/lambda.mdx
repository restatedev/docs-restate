...
- `RESTATE_AUTH_TOKEN` to your Restate Cloud or BYOC auth token. To get one, go to [Developers > API Keys > Create API Key](https://cloud.restate.dev/to/developers/integration?createApiKey=true&createApiKeyDescription=deployment-key&createApiKeyRole=rst:role::FullAccess), and make sure to select **Full** for the role
...
- `AWS_INVOKE_ROLE_TO_ASSUME`: The role created in [Deploy and allow Restate to invoke the function](#deploy-and-allow-restate-to-invoke-the-function)
