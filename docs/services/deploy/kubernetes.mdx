...
<AccordionGroup>
    ...
    <Accordion title="How does the standalone tunnel client work?">
        Restate Cloud sends invocations over an outbound connection maintained by the tunnel client pod created by the `RestateCloudEnvironment`. The tunnel client pod forwards each invocation through a Kubernetes Service to your application pod.
