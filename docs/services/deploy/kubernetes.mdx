...
kubectl get restatecloudenvironment my-cloud-environment
```
...
<Steps>
    <Step title="Download the TypeScript template">
        Either start from an existing Restate service, or download the TypeScript Kubernetes template from the Restate examples repository:

```bash
restate example typescript-kubernetes && cd typescript-kubernetes
```
    ...
    <Step title="Configure the Restate API key">
        Optionally, create the namespace where your services will run:
    ...
    <Tab title="Go">
        <Steps>
            <Step title="Download the Go template">
                Either start from an existing Restate service, or download the Go Kubernetes template from the Restate examples repository:

```bash
restate example go-kubernetes && cd go-kubernetes
```
...
```
            ...
            <Step title="Invoke the service">
                Open the Restate Cloud UI, select the `Greeter` service, and invoke its `greet` handler from the Playground with:
...
```
    ...
    <Tab title="Rust">
        <Steps>
            <Step title="Download the Rust template">
                Either start from an existing Restate service, or download the Rust template from the Restate examples repository:
...
Restate allows you to scale services horizontally by running multiple pod replicas. How traffic reaches those replicas depends on the deployment mode:

- With an in-process Restate Cloud tunnel, every pod opens its own outbound connection. The tunnel distributes invocations across the replicas, so you do not need a Kubernetes Service, load balancer, or proxy in the request path.
- With a standalone tunnel, direct Kubernetes deployment, or self-hosted Restate, use a Kubernetes Service or another load balancer to provide a stable endpoint for the pods.
...
When the service is reached over the Kubernetes network, you would typically use a `Service` of type `ClusterIP` or `LoadBalancer` to distribute traffic across your pods.
