...
<Tip>
    Follow the [Restate Deno Deploy quickstart](/quickstart) to set up your project.
</Tip>
...
You can also add Restate into an existing Deno project. The SDK should be imported with the `fetch` component, for example: <code>{<CustomVars name="DENO_FETCH"/>}</code>.
