...
Make sure your `wrangler.toml` has the `nodejs_compat` flag, minification is disabled and and preview urls are enabled:
...
In order to make sure only a specific Restate Cloud environment can push requests to your Cloudflare Workers deployment,
...
You can set up automation to deploy to Cloudflare Workers on each commit to `main` and automatically register new [Restate service versions](/services/versioning):
