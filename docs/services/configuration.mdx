...
If you use the [Spring Boot integration](https://docs.restate.dev/develop/java/serving), you can configure services directly from `application.properties` or `application.yml` instead of modifying code.

**Global defaults** apply to all registered services. Per-service and per-handler properties override them:

```properties
# Global defaults — applied to every service
...
The full set of supported properties at each level:

| Property | Description |
|---|---|
| `inactivity-timeout` | Time Restate waits for new journal entries before suspending the handler |
| `abort-timeout` | Time before forcibly aborting after the inactivity timeout |
| `idempotency-retention` | Retention period for idempotency keys |
| `workflow-retention` | Retention period after a workflow run handler completes |
| `journal-retention` | Retention period for invocation journals |
| `ingress-private` | Whether to block external HTTP/Kafka ingress |
| `enable-lazy-state` | Whether to use lazy K/V state loading |
| `retry-policy.initial-interval` | Initial retry interval |
| `retry-policy.exponentiation-factor` | Retry backoff factor |
| `retry-policy.max-interval` | Maximum retry interval |
| `retry-policy.max-attempts` | Maximum number of retry attempts |
| `retry-policy.on-max-attempts` | Action on exhausted retries (`PAUSE` or `KILL`) |
| `documentation` | Service/handler documentation string |
| `metadata.*` | Arbitrary string metadata key/value pairs |

The `executor` property is only available at the global level (`restate.executor`) and per-service level (`restate.components.<Name>.executor`). It is not supported as a per-handler property.
