...
<Accordion title="Spring Boot: configuring all services at once">

When using the Spring Boot integration, you can set **global defaults** in `application.properties` or `application.yml` that apply to every registered service. Per-service properties under `restate.components.<Name>.*` override the global defaults.

```properties
# Applied to every registered service by default
...
All properties supported under `restate.components.<Name>.*` can be used as global defaults at the top-level `restate.*` — except `executor`, which was already a global-only property.

**Supported global default properties:**

| Property | Description |
|---|---|
| `restate.inactivity-timeout` | Maximum time to wait for new journal entries before suspending |
| `restate.abort-timeout` | Time to wait before forcibly aborting after inactivity timeout |
| `restate.idempotency-retention` | How long to retain idempotency keys |
| `restate.workflow-retention` | How long workflow state is kept after completion |
| `restate.journal-retention` | How long invocation journals are kept after completion |
| `restate.ingress-private` | Prevent external HTTP/Kafka access; allow only internal service-to-service calls |
| `restate.enable-lazy-state` | Use lazy state loading instead of the default eager strategy |
| `restate.retry-policy.*` | Retry policy (initial interval, max attempts, exponentiation factor, etc.) |
| `restate.documentation` | Documentation string shown in the UI |
| `restate.metadata.*` | Arbitrary key-value metadata |
