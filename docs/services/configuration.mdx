...
If you use the Java/Kotlin SDK with Spring Boot, you can configure services using Spring's `application.properties` or `application.yml` files instead of (or in addition to) code-based configuration.
...
**Global defaults (v2.7.0+):**

Top-level `restate.*` properties act as defaults for all registered services. Per-service `restate.components.<Name>.*` properties override them:
...
restate.components.MyWorkflow.handlers.run.workflow-retention=30d
```
