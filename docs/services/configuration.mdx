...
### Spring Boot: global default configuration

When using the Restate Spring Boot starter, you can set global defaults for all services using top-level `restate.*` properties. Per-service `restate.components.<ServiceName>.*` properties override the defaults on a per-service basis:
...
Per-service `restate.components.<ServiceName>.*` properties always take precedence over global defaults. The `executor` property is only available at the global level (`restate.executor`) and per-service level (`restate.components.<ServiceName>.executor`), not as a top-level default.
