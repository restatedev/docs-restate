...
If you use the Java/Kotlin SDK with Spring Boot, you can configure services from `application.properties` or `application.yml` without modifying code.
...
**Per-service overrides** (override global defaults for a specific service):
...
All options available on `restate.components.<Name>.*` are also supported as top-level defaults under `restate.*`:
...
<Note>
  `executor` can only be set globally at `restate.executor`, not per-service via global defaults.
</Note>

You can also configure a service programmatically using `RestateComponentProperties`:
