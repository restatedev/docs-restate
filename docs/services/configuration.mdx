...
When using the Spring Boot integration, top-level `restate.*` properties act as defaults applied to all registered services. Per-service `restate.components.<Name>.*` properties override them for a specific service:
...
The following properties are supported as global defaults: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `retry-policy`.

This is useful when you want to apply consistent defaults across all services without repeating configuration for each one. Per-service and per-handler configuration under `restate.components.<Name>.*` always takes precedence.

<Info>
  The `executor` property is not supported as a global default via `restate.*`. It is configured globally via `restate.executor` and per-service via `restate.components.<Name>.executor`.
</Info>
