...
If you use the [Spring Boot integration](https://docs.restate.dev/get_started/quickstart?sdk=java), you can configure services through Spring's `application.properties` or `application.yml` files instead of writing code.

**Per-service configuration** (`restate.components.<Name>.*`):
...
**Global defaults** (`restate.*`): Top-level `restate.*` properties act as defaults applied to all registered services. Per-service `restate.components.<Name>.*` properties override them on a per-service basis:
...
<Note>
    The `executor` property is not supported as a global default via top-level `restate.*`. Use `restate.executor` (which was already a global default) or `restate.components.<Name>.executor` per service.
</Note>
