...
### Spring Boot: global defaults and per-service configuration

When using the [Spring Boot integration](/quickstart?sdk=java), you can configure service options in `application.properties` or `application.yml`.
...
Supported properties (both at the top level and per-service):
...
You can also configure per-handler options:
...
Alternatively, use the `@RestateService` annotation with a `RestateComponentProperties` bean:
...
public RestateComponentProperties myServiceConfig() {
    return new RestateComponentProperties()
        ...
        .setJournalRetention(Duration.ofDays(7));
}
```
