...
### Spring Boot: global defaults and per-service properties

If you use the [Spring Boot integration](https://github.com/restatedev/sdk-java/tree/main/sdk-spring-boot-starter), you can configure services via `application.properties` or `application.yml` without modifying code. Top-level `restate.*` properties act as defaults for all registered services. Per-service `restate.components.<Name>.*` properties override them for a specific service.
...
The full list of supported global defaults mirrors the per-service properties: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, and `retry-policy`. The `executor` property is global-only and already applied globally via `restate.executor`.

Per-service `restate.components.<Name>.*` properties always take precedence over the global defaults.
