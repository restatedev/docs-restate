...
If you use the Spring Boot integration, you can configure services directly in `application.properties` or `application.yml` without modifying code.

Top-level `restate.*` properties act as defaults applied to all registered services. Per-service `restate.components.<Name>.*` properties override those defaults for a specific service.
...
Supported global defaults: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `retry-policy`. Per-service properties also support `executor` and per-handler configuration under `restate.components.<Name>.handlers.<handlerName>.*`.

<Note>
    Global default properties (`restate.*`) require Java/Kotlin SDK >= 2.7.0. Per-service properties (`restate.components.*`) are available since SDK >= 2.6.0.
</Note>
