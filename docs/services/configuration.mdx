...
### Spring Boot: configure via application properties

If you use the [Spring Boot integration](https://docs.restate.dev/develop/java/serving), you can configure service options directly from `application.properties` or `application.yml` instead of in code.
...
**Per-service overrides** — `restate.components.<ServiceName>.*` override the global defaults for a specific service:
...
**Per-handler overrides** — `restate.components.<ServiceName>.handlers.<handlerName>.*`:
...
All properties available at the service level (`inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `metadata`, `documentation`, `retry-policy`) are supported as global defaults, except `executor` which remains a global default at `restate.executor`.

<Note>
    Global default service properties require Java/Kotlin SDK >= 2.7.0.
    Per-service properties require Java/Kotlin SDK >= 2.6.0.
</Note>
