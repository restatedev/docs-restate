...
### Spring Boot: global and per-service configuration

When using the Spring Boot integration, you can configure services from `application.properties` or `application.yml`.

**Global defaults** — top-level `restate.*` properties apply to all registered services:

```properties
# Applied to every service by default
...
restate.retry-policy.max-interval=60s
restate.retry-policy.on-max-attempts=PAUSE
```

**Per-service overrides** — `restate.components.<ServiceName>.*` properties override the global defaults for a specific service:

```properties
# Overrides only for MyService
restate.components.MyService.journal-retention=PT72H
restate.components.MyService.inactivity-timeout=PT30M
restate.components.MyService.ingress-private=true
```

**Per-handler overrides** — `restate.components.<ServiceName>.handlers.<handlerName>.*` properties apply to individual handlers:

```properties
restate.components.MyService.handlers.myHandler.inactivity-timeout=PT5M
restate.components.MyService.handlers.myWorkflowHandler.workflow-retention=PT720H
```

The full list of supported properties for both global defaults and per-service configuration: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `retry-policy`.

The `executor` property is only supported per-service (`restate.components.<Name>.executor`) and as a top-level global (`restate.executor`), not as a named default.
