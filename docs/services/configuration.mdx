...
When using the Spring Boot integration, top-level `restate.*` properties act as defaults applied to all registered services. Per-service `restate.components.<Name>.*` properties override them on a per-service basis.

```properties
# Applied to all services by default
restate.journal-retention=PT48H
restate.inactivity-timeout=PT10M
restate.retry-policy.initial-interval=PT0.1S
...
The `executor` property is only configurable per service via `restate.components.<Name>.executor` (or globally as `restate.executor`), not as a top-level default.
