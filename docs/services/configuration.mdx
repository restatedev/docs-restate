...
If you use the Restate Spring Boot integration, you can configure defaults that apply to all services via top-level `restate.*` properties in `application.properties` or `application.yml`. Per-service settings under `restate.components.<Name>.*` override these defaults:
...
# Override only for MyService
restate.components.MyService.journal-retention=PT72H
restate.components.MyService.ingress-private=true
```

All properties available on `restate.components.<Name>.*` are supported as top-level defaults, including: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, and `retry-policy`.

<Info>
    The `executor` property is not inherited from the global defaults; it remains a global setting under `restate.executor`.
</Info>
