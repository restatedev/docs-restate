...
When using the Spring Boot integration, you can set top-level `restate.*` properties in `application.properties` or `application.yml` to apply default configuration to **all** registered services. Per-service `restate.components.<Name>.*` properties override these defaults for a specific service.
...
# Overrides the defaults for MyService only
restate.components.MyService.journal-retention=PT72H
restate.components.MyService.ingress-private=true
```

All `restate.components.<Name>.*` properties are supported as global defaults, including:

| Property | Description |
|---|---|
| `inactivity-timeout` | Time before Restate considers an invocation stalled |
| `abort-timeout` | Time before Restate forcibly aborts execution |
| `idempotency-retention` | How long idempotency keys are retained |
| `workflow-retention` | How long workflow state is retained after completion |
| `journal-retention` | How long the invocation journal is retained |
| `ingress-private` | Prevent the service from receiving external requests |
| `enable-lazy-state` | Use lazy state loading instead of eager |
| `retry-policy.*` | Retry policy settings (`max-attempts`, `initial-interval`, etc.) |
| `documentation` | Service documentation string |
| `metadata.*` | Arbitrary key-value metadata for the service |

Note: `restate.executor` (thread pool configuration) was already a global default before v2.7.0 and is not affected by this change.
