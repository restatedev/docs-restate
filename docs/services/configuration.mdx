...
If you use the Spring Boot integration, you can configure services via `application.properties` or `application.yml` without modifying code. Top-level `restate.*` properties act as defaults for all registered services. Per-service `restate.components.<Name>.*` properties override them for a specific service.
