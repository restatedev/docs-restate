...
When using the [Spring Boot integration](/develop/java/serving), top-level `restate.*` properties in `application.properties` or `application.yml` act as **defaults applied to all registered services**. Per-service `restate.components.<Name>.*` properties override them for individual services.
...
# Overrides defaults only for MyService
...
The `executor` property is only available as a per-service override (`restate.components.<Name>.executor`) or as a global JVM-level setting (`restate.executor`) — not as part of the above global defaults.
