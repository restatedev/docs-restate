...
If you use the [Spring Boot integration](/quickstart#java), you can configure services via `application.properties` or `application.yml`.

**Global defaults** — top-level `restate.*` properties apply to every registered service:

```properties application.properties
# Applied to all services by default
restate.journal-retention=PT48H
restate.inactivity-timeout=PT10M
restate.retry-policy.initial-interval=100ms
restate.retry-policy.exponentiation-factor=2.0
restate.retry-policy.max-interval=10s
restate.retry-policy.max-attempts=5
restate.retry-policy.on-max-attempts=PAUSE
```

**Per-service overrides** — use `restate.components.<ServiceName>.*` to override the global defaults for a specific service:
...
# Overrides only for MyService
restate.components.MyService.journal-retention=PT72H
restate.components.MyService.ingress-private=true
```

The full set of supported properties (at both global and per-service level):

| Property | Description |
|---|---|
| `inactivity-timeout` | Inactivity timeout for handlers (ISO-8601 duration) |
| `abort-timeout` | Abort timeout for handlers (ISO-8601 duration) |
| `idempotency-retention` | Idempotency retention period |
| `workflow-retention` | Workflow retention period |
| `journal-retention` | Journal retention period |
| `ingress-private` | Whether to make the service private |
| `enable-lazy-state` | Enable lazy state loading |
| `retry-policy.initial-interval` | Initial retry interval |
| `retry-policy.exponentiation-factor` | Retry backoff exponentiation factor |
| `retry-policy.max-interval` | Maximum retry interval |
| `retry-policy.max-attempts` | Maximum retry attempts |
| `retry-policy.on-max-attempts` | What to do after exhausting attempts (`PAUSE` or `KILL`) |
| `documentation` | Service documentation string |
| `metadata.*` | Service metadata key-value pairs |

**Per-handler overrides** — use `restate.components.<ServiceName>.handlers.<handlerName>.*`:

```properties application.properties
restate.components.MyService.handlers.myHandler.inactivity-timeout=PT30M
restate.components.MyService.handlers.myHandler.ingress-private=true
```

<Note>
    The global `restate.executor` property (Java only) sets the default executor for all services. Per-service `restate.components.<Name>.executor` overrides it for a specific service.
</Note>
