...
### Spring Boot: configuring services via application properties

If you use the Spring Boot integration, you can configure service options in your `application.properties` or `application.yml` files instead of (or in addition to) code.

**Global defaults** apply to all registered services. **Per-service overrides** apply only to the named service and take precedence over the global defaults.
...
# Override specific properties only for MyService
...
The supported global defaults are: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, and `retry-policy`. The `executor` property can only be set at the global level (`restate.executor`) or per-service (`restate.components.<Name>.executor`).

<Tip>
    Global defaults are applied first; per-service `restate.components.<Name>.*` values override them for that specific service.
</Tip>
