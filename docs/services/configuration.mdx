...
If you use the Restate Spring Boot integration, you can configure services via `application.properties` or `application.yml` instead of (or in addition to) the programmatic API.

**Per-service configuration** uses the `restate.components.<ServiceName>.*` namespace:

```properties
restate.components.MyService.inactivity-timeout=PT10M
restate.components.MyService.abort-timeout=PT1M
restate.components.MyService.journal-retention=PT48H
restate.components.MyService.idempotency-retention=PT168H
restate.components.MyService.workflow-retention=PT720H
restate.components.MyService.ingress-private=true
restate.components.MyService.enable-lazy-state=false
...
**Per-handler configuration** uses `restate.components.<ServiceName>.handlers.<handlerName>.*`:
...
**Global defaults** (since v2.7.0): top-level `restate.*` properties act as defaults applied to all registered services. Per-service properties override them:
