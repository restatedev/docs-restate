...
When using the Spring Boot integration (Java/Kotlin), you can set default configuration that applies to all registered services from your `application.properties` or `application.yml`.

Top-level `restate.*` properties act as defaults for every service. Per-service `restate.components.<Name>.*` properties override them for that specific service.
...
Note: `executor` is already a global default at `restate.executor` and is not included above.
