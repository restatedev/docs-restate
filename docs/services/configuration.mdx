...
When using the Spring Boot integration, you can configure services directly from `application.properties` or `application.yml`.

**Per-service configuration** via `restate.components.<Name>.*`:

```properties
restate.components.MyService.inactivity-timeout=PT10M
restate.components.MyService.abort-timeout=PT1M
restate.components.MyService.journal-retention=PT24H
restate.components.MyService.idempotency-retention=PT168H
restate.components.MyService.workflow-retention=PT24H
...
**Global defaults** (SDK >= 2.7.0): top-level `restate.*` properties act as defaults for all registered services. Per-service properties override them:
...
# Overrides just for MyService (takes precedence over global defaults)
restate.components.MyService.journal-retention=PT72H
restate.components.MyService.ingress-private=true
```

Supported global defaults: `documentation`, `metadata`, `inactivity-timeout`, `abort-timeout`, `idempotency-retention`, `workflow-retention`, `journal-retention`, `ingress-private`, `enable-lazy-state`, `retry-policy`. The `executor` property is a global-only setting via `restate.executor`.

You can also configure services using a bean via `@RestateService(configuration = "beanName")`:
