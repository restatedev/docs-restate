...
When using the Spring Boot integration, you can configure services using Spring's `application.properties` or `application.yml` instead of code.

**Global defaults** apply to all registered services. Per-service properties under `restate.components.<Name>.*` override the defaults for that specific service:
...
You can also configure individual handlers:

```properties
# Per-handler overrides for MyService
restate.components.MyService.handlers.myHandler.inactivity-timeout=PT5M
restate.components.MyService.handlers.myWorkflowHandler.workflow-retention=P30D
```

Alternatively, configure services programmatically using the `configuration` attribute on `@RestateService`, `@RestateVirtualObject`, or `@RestateWorkflow`:
...
public RestateComponentProperties myServiceConfig() {
    return new RestateComponentProperties()
        ...
        .setJournalRetention(Duration.ofDays(3));
}
```
