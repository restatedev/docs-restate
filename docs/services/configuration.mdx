...
### Spring Boot: global default configuration (Java/Kotlin)

If you use the Spring Boot integration, you can set default configuration for all services at once using top-level `restate.*` properties. These act as defaults applied to every registered service. Per-service `restate.components.<Name>.*` properties override them on a per-service basis.
...
Per-service properties in `restate.components.<Name>.*` always take precedence over the top-level defaults.

<Note>
    The `executor` property is already a global default at `restate.executor` and is not affected by this change.
</Note>
