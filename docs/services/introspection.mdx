...
<Accordion title="Retrieving the status of an invocation">
    ...
    * `scheduled`: scheduled to run at a future time (e.g. a delayed send)
    * `ready`: ready to be processed, but not yet running
    * `running`: actively processing
    * `paused`: manually paused (resume with `restate invocations resume <id>`)
    * `backing-off`: retrying due to a failure
    * `suspended`: waiting on some external input (e.g. request-response call, awakeable, sleep, ...)
    * `completed`: completed (this is shown only for idempotent invocations and workflows)

    Cancellation and kill are **actions**, not statuses. An invocation that is being cancelled or killed transitions to `completed` once Restate has finished tearing it down; the outcome is reflected in `completion_result` (`success` or `failure`) and, for failures, `completion_failure`. See [Managing invocations](/services/invocation/managing-invocations) for the cancel/kill workflow.
</Accordion>
