...
## Applying concurrency limits from within a handler

In addition to the HTTP ingress endpoints, you can route calls through a scope from within a handler using the `ctx.scope()` API (TypeScript SDK v1.15.0+).

<Note title="Preview API">
    `ctx.scope()` requires restate-server 1.7 with both `RESTATE_EXPERIMENTAL_ENABLE_PROTOCOL_V7=true` and `RESTATE_EXPERIMENTAL_ENABLE_VQUEUES=true` enabled.
    If these experimental features are not enabled, the call fails with a retryable error.
</Note>

`ctx.scope(scopeKey)` returns a `ScopedContext` that routes all outgoing calls within that scope.
The scope key must contain only `[a-zA-Z0-9_.-]` characters and be between 1 and 36 characters long.
...
You can also add a `limitKey` to invocations for hierarchical concurrency control within a scope.
The scope key selects which rule book rule applies; the limit key further subdivides the concurrency budget within that scope.

The `scope` and `limitKey` values are also available on the incoming request via `ctx.request().scope` and `ctx.request().limitKey`.
