...
<Note title="Experimental feature">
    Flow control is an experimental feature and is disabled by default.
...
<Note title="SDK support">
    Today, you apply flow control by invoking handlers through the [scoped ingress endpoints](#applying-concurrency-limits).
    Support for setting a scope directly from the SDKs will be added over the coming weeks.
</Note>
...
- `*` matches every scope and acts as a global default.
