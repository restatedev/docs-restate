...
<Note title="Preview">
  `ctx.scope(...)` and `ingress.scope(...)` are in preview. Enable them in the server with
  `RESTATE_EXPERIMENTAL_ENABLE_PROTOCOL_V7=true`, `RESTATE_EXPERIMENTAL_ENABLE_VQUEUES=true`, and
  `RESTATE_EXPERIMENTAL_ENABLE_SCOPED_VIRTUAL_OBJECTS=true` (for virtual object clients only).
</Note>

Use `ctx.scope(scopeKey)` inside a handler to route a call into a scope:

```typescript
// Services
await ctx.scope("tenant-123").serviceClient(MyService).process(payload);

// Virtual Objects (requires RESTATE_EXPERIMENTAL_ENABLE_SCOPED_VIRTUAL_OBJECTS=true)
await ctx.scope("tenant-123").objectClient(MyObject, "obj-key").update(payload);
ctx.scope("tenant-123").objectSendClient(MyObject, "obj-key").update(payload);
...
Use `ingress.scope(scopeKey)` on the ingress client to apply a scope from outside a handler:
...
// Services
await ingress.scope("checkout").serviceClient(MyService).checkout(request);
