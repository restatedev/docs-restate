...
<Expandable title="Option 4: Store metadata on AWS DynamoDB.">
        <ResponseField name="type" type="string" post={['enum: "dynamo-db"']}>
...
Set `type: "dynamo-db"`
        ...
        <ResponseField name="table" type="string" post={[]}>
            DynamoDB table name or ARN: The DynamoDB table must have a string partition key named `pk`.

        </ResponseField>

        <ResponseField name="aws-profile" type="string | null" post={[]}>
            AWS profile: The AWS configuration profile to use for DynamoDB. If you use named profiles in your AWS configuration, you can replace all the other settings with a single profile reference. See the [AWS documentation on profiles](https://docs.aws.amazon.com/sdkref/latest/guide/file-format.html) for more.

        </ResponseField>

        <ResponseField name="aws-region" type="string | null" post={[]}>
            AWS region: AWS region to use with DynamoDB. This may be inferred from the environment, for example the current region when running in EC2.

        </ResponseField>

        <ResponseField name="aws-access-key-id" type="string | null" post={[]}>
            AWS access key: Used for authentication with DynamoDB.

        </ResponseField>

        <ResponseField name="aws-secret-access-key" type="string | null" post={[]}>
            AWS secret key: Used for authentication with DynamoDB.
        ...
        <ResponseField name="key-prefix" type="string | null" post={[]}>
            Key prefix: Optional prefix for all keys in DynamoDB. Useful for sharing a single table across multiple Restate clusters.

        </ResponseField>

        <ResponseField name="aws-endpoint-url" type="string | null" post={[]}>
            DynamoDB API endpoint URL override: Typically inferred from the region. Use this for testing with DynamoDB Local.
