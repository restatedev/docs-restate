...
<ResponseField name="worker" type="object">
    ...
    <Expandable title="Properties">
        <ResponseField name="internal-queue-length" type="integer">
            Internal queue for partition processor communication (default: `1000`)
        </ResponseField>

        <ResponseField name="num-timers-in-memory-limit" type="integer">
            The number of timers in memory limit is used to bound the amount of timers loaded in memory. If this limit is set, when exceeding it, the timers farther in the future will be spilled to disk. (default: `null`)
        </ResponseField>
...
Can be configured using the [`humantime`](https://docs.rs/humantime/latest/humantime/fn.parse_duration.html) format. (default: `"1h"`)
        </ResponseField>
...
This defines the total memory for rocksdb as a ratio of all memory available to memtables (See `rocksdb-total-memtables-ratio` in common). The budget is then divided evenly across partitions. The divisor is defined in `num-partitions-to-share-memory-budget` (default: `0.49000000953674316`)
                </ResponseField>
...
Can be configured using the [`humantime`](https://docs.rs/humantime/latest/humantime/fn.parse_duration.html) format. (default: `"1m"`)
                </ResponseField>
...
Can be configured using the [`humantime`](https://docs.rs/humantime/latest/humantime/fn.parse_duration.html) format. (default: `"1m"`)
                ...
                <ResponseField name="message-size-limit" type="anyOf">
                    Threshold to fail the invocation in case protocol messages coming from a service are larger than the specified amount. (default: `null`)
                ...
                <ResponseField name="tmp-dir" type="string">
                    Temporary directory to use for the invoker temporary files. If empty, the system temporary directory will be used instead. (default: `null`)
                </ResponseField>

                <ResponseField name="in-memory-queue-length-limit" type="integer">
                    Defines the threshold after which queues invocations will spill to disk at the path defined in `tmp-dir`. In other words, this is the number of invocations that can be kept in memory before spilling to disk. This is a per-partition limit. (default: `66049`)
                </ResponseField>

                <ResponseField name="concurrent-invocations-limit" type="integer">
                    Number of concurrent invocations that can be processed by the invoker. (default: `1000`)
                </ResponseField>
        ...
        <ResponseField name="max-command-batch-size" type="integer">
            The maximum number of commands a partition processor will apply in a batch. The larger this value is, the higher the throughput and latency are. (default: `32`)
        </ResponseField>
...
Default: `None` - automatic snapshots are disabled (default: `null`)
                </ResponseField>
...
<ResponseField name="admin" type="object">
    ...
    <Expandable title="Properties">
        <ResponseField name="bind-address" type="string">
            Address to bind for the Admin APIs. (default: `"0.0.0.0:9070"`)
        </ResponseField>

        <ResponseField name="advertised-admin-endpoint" type="string">
            Optional advertised Admin API endpoint. (default: `null`)
        </ResponseField>

        <ResponseField name="concurrent-api-requests-limit" type="integer">
            Concurrency limit for the Admin APIs. Default is unlimited. (default: `null`)
        </ResponseField>
        ...
        <ResponseField name="query-engine" type="object">
            ...
            <Expandable title="Properties">
                ...
                <ResponseField name="tmp-dir" type="string">
                    The path to spill to (default: `null`)
                </ResponseField>

                <ResponseField name="query-parallelism" type="integer">
                    The number of parallel partitions to use for a query execution (default: `null`)
                </ResponseField>
        ...
        <ResponseField name="heartbeat-interval" type="string">
            Controls the interval at which cluster controller polls nodes of the cluster. (default: `"1s 500ms"`)
        </ResponseField>

        <ResponseField name="log-trim-check-interval" type="string">
...
<ResponseField name="ingress" type="object">
    ...
    <Expandable title="Properties">
        <ResponseField name="bind-address" type="string">
            The address to bind for the ingress. (default: `"0.0.0.0:8080"`)
        </ResponseField>

        <ResponseField name="concurrent-api-requests-limit" type="integer">
            Local concurrency limit to use to limit the amount of concurrent requests. If exceeded, the ingress will reply immediately with an appropriate status code. Default is unlimited. (default: `null`)
        </ResponseField>
        ...
        <ResponseField name="advertised-ingress-endpoint" type="string">
            Ingress endpoint that the Web UI should use to interact with. (default: `null`)
        </ResponseField>
...
<ResponseField name="bifrost" type="object">
    ...
    <Expandable title="Properties">
        ...
        <ResponseField name="local" type="string">
            Configuration of local loglet provider (default: `{"rocksdb-disable-wal":false,"rocksdb-disable-wal-fsync":false,"rocksdb-log-keep-file-num":null,"rocksdb-log-level":null,"rocksdb-log-max-file-size":null,"rocksdb-memory-ratio":0.5,"writer-batch-commit-count":5000,"writer-batch-commit-duration":"0s"}`)
        </ResponseField>
...
Note that this will be increased to fit the biggest batch of records being enqueued. (default: `1000`)
                </ResponseField>
...
The sequencer is also allowed to use this value as interval to send seal/release checks even if it's not quiescent. (default: `"15s"`)
                </ResponseField>

                <ResponseField name="log-server-rpc-timeout" type="string">
                    Log Server RPC timeout

Timeout waiting on log server response (default: `"2s"`)
                </ResponseField>
...
The number of records bifrost will attempt to prefetch from replicated loglet's log-servers for every loglet reader (e.g. partition processor). Note that this mainly impacts readers that are not co-located with the loglet sequencer (i.e. partition processor followers). (default: `100`)
                </ResponseField>
...
Value must be between 0 and 1. It will be clamped at `1.0`. (default: `0.5`)
                </ResponseField>
...
Can be configured using the [`humantime`](https://docs.rs/humantime/latest/humantime/fn.parse_duration.html) format.
        ...
        <ResponseField name="seal-retry-interval" type="string">
            Interval to wait between retries of loglet seal failures (default: `"2s"`)
        </ResponseField>

        <ResponseField name="auto-recovery-interval" type="string">
            Time interval after which bifrost's auto-recovery mechanism will kick in. This is triggered in scenarios where the control plane took too long to complete loglet reconfigurations. (default: `"15s"`)
        </ResponseField>

        <ResponseField name="append-retry-min-interval" type="string">
            Minimum retry duration used by the exponential backoff mechanism for bifrost appends. (default: `"10ms"`)
        </ResponseField>

        <ResponseField name="append-retry-max-interval" type="string">
            Maximum retry duration used by the exponential backoff mechanism for bifrost appends. (default: `"1s"`)
        </ResponseField>
...
This allows the log to pick up replication property changes, apply better placement of replicas, or for other reasons. (default: `false`)
        </ResponseField>
...
Number of in-flight metadata store requests. (default: `32`)
        </ResponseField>
...
This defines the total memory for rocksdb as a ratio of all memory available to memtables (See `rocksdb-total-memtables-ratio` in common). (default: `0.009999999776482582`)
        </ResponseField>
...
<ResponseField name="networking" type="object">
    ...
    <Expandable title="Properties">
        <ResponseField name="connect-timeout" type="string">
            TCP connection timeout for Restate cluster node-to-node network connections. (default: `"3s"`)
        </ResponseField>
...
Can be configured using the [`humantime`](https://docs.rs/humantime/latest/humantime/fn.parse_duration.html) format.
        ...
        <ResponseField name="handshake-timeout" type="string">
            Timeout for receiving a handshake response from Restate cluster peers. (default: `"3s"`)
        </ResponseField>

        <ResponseField name="http2-keep-alive-interval" type="string">
            HTTP/2 Keep Alive Interval (default: `"1s"`)
        </ResponseField>

        <ResponseField name="http2-keep-alive-timeout" type="string">
            HTTP/2 Keep Alive Timeout (default: `"3s"`)
        </ResponseField>

        <ResponseField name="http2-adaptive-window" type="boolean">
            HTTP/2 Adaptive Window (default: `true`)
        </ResponseField>
...
(See `rocksdb-total-memtables-ratio` in common). (default: `0.5`)
        </ResponseField>

        <ResponseField name="rocksdb-disable-wal-fsync" type="boolean">
            Disable fsync of WAL on every batch (default: `false`)
        </ResponseField>
...
Default is 0 which maps to floor(number of CPU cores / 2) (default: `0`)
        </ResponseField>
...
Set to 0 or 1 to commit the write batch on every command. (default: `5000`)
        </ResponseField>

        <ResponseField name="incoming-network-queue-length" type="integer">
            The number of messages that can queue up on input network stream while request processor is busy. (default: `1000`)
        </ResponseField>
...
<ResponseField name="roles" type="array">
    Defines the roles which this Restate node should run, by default the node starts with all roles. (default: `["worker","admin","metadata-server","log-server","http-ingress"]`)
...
<ResponseField name="node-name" type="string">
    Unique name for this node in the cluster. The node must not change unless it's started with empty local store. It defaults to the node's hostname. (default: `null`)
</ResponseField>
...
<ResponseField name="force-node-id" type="integer">
    If set, the node insists on acquiring this node ID. (default: `null`)
</ResponseField>

<ResponseField name="cluster-name" type="string">
    A unique identifier for the cluster. All nodes in the same cluster should have the same. (default: `"localcluster"`)
</ResponseField>
...
<ResponseField name="base-dir" type="string">
    The working directory which this Restate node should use for relative paths. The default is `restate-data` under the current working directory. (default: `null`)
</ResponseField>
...
<ResponseField name="metadata-client" type="object">
    ...
    <Expandable title="Properties">
        <ResponseField name="connect-timeout" type="string">
            TCP connection timeout for connecting to the metadata store. (default: `"3s"`)
        </ResponseField>

        <ResponseField name="keep-alive-interval" type="string">
            Metadata Store Keep Alive Interval (default: `"5s"`)
        </ResponseField>

        <ResponseField name="keep-alive-timeout" type="string">
            Metadata Store Keep Alive Timeout (default: `"5s"`)
        </ResponseField>
...
<ResponseField name="advertised-address" type="string">
    Address that other nodes will use to connect to this node. Default is `http://127.0.0.1:5122/` (default: `"http://127.0.0.1:5122/"`)
</ResponseField>
...
To update existing clusters use the `restatectl` utility. (default: `1`)
</ResponseField>
...
<ResponseField name="default-thread-pool-size" type="integer">
    Size of the default thread pool used to perform internal tasks. If not set, it defaults to the number of CPU cores. (default: `null`)
</ResponseField>

<ResponseField name="log-filter" type="string">
    Log filter configuration. Can be overridden by the `RUST_LOG` environment variable. Check the [`RUST_LOG` documentation](https://docs.rs/tracing-subscriber/latest/tracing_subscriber/filter/struct.EnvFilter.html) for more details how to configure it. (default: `"warn,restate=info"`)
</ResponseField>
...
<ResponseField name="log-disable-ansi-codes" type="boolean">
    Disable ANSI terminal codes for logs. This is useful when the log collector doesn't support processing ANSI terminal codes. (default: `false`)
</ResponseField>
...
<ResponseField name="histogram-inactivity-timeout" type="string">
    Timeout for idle histograms.

The duration after which a histogram is considered idle and will be removed from metric responses to save memory. This value should be configured higher than the scrape interval of the telemetry collection system (e.g. Prometheus). (default: `"3m"`)
</ResponseField>

<ResponseField name="disable-prometheus" type="boolean">
    Disable prometheus metric recording and reporting. Default is `false`. (default: `false`)
</ResponseField>
...
<ResponseField name="rocksdb-high-priority-bg-threads" type="integer">
    The number of threads to reserve to high priority Rocksdb background tasks. (default: `2`)
</ResponseField>

<ResponseField name="rocksdb-write-stall-threshold" type="string">
    This defines the duration after which a write is to be considered in \"stall\" state. For every write that meets this threshold, the system will increment the `restate.rocksdb_stall_flare` gauge, if the write is unstalled, the guage will be updated accordingly. (default: `"3s"`)
</ResponseField>

<ResponseField name="rocksdb-enable-stall-on-memory-limit" type="boolean">
    Note if automatic memory budgeting is enabled, it should be safe to allow rocksdb to stall if it hits the limit. However, if rocksdb stall kicked in, it's unlikely that the system will recover from this without intervention. (default: `false`)
</ResponseField>
...
<ResponseField name="metadata-update-interval" type="string">
    The idle time after which the node will check for metadata updates from metadata store. This helps the node detect if it has been operating with stale metadata for extended period of time, primarily because it didn't interact with other peers in the cluster during that period. (default: `"10s"`)
</ResponseField>

<ResponseField name="metadata-fetch-from-peer-timeout" type="string">
    When a node detects that a new metadata version exists, it'll attempt to fetch it from its peers. After this timeout duration has passed, the node will attempt to fetch the metadata from metadata store as well. This is to ensure that the nodes converge quickly while reducing the load on the metadata store. (default: `"3s"`)
</ResponseField>
...
<ResponseField name="initialization-timeout" type="string">
    The timeout until the node gives up joining a cluster and initializing itself. (default: `"5m"`)
</ResponseField>

<ResponseField name="disable-telemetry" type="boolean">
    Restate uses Scarf to collect anonymous usage data to help us understand how the software is being used. You can set this flag to true to disable this collection. It can also be set with the environment variable DO_NOT_TRACK=1. (default: `false`)
</ResponseField>
...
This file is currently only read on client creation, but this may change in future. Parsed public keys will be logged at INFO level in the same format that SDKs expect. (default: `null`)
</ResponseField>
...
You should set this timeout with a value lower than the `abort_timeout`. (default: `"40s"`)
        </ResponseField>

        <ResponseField name="timeout" type="string">
            Sets a timeout for receiving an acknowledgement of the keep-alive ping.

If the ping is not acknowledged within the timeout, the connection will be closed. (default: `"20s"`)
        </ResponseField>
...
<ResponseField name="http-proxy" type="string">
    A URI, such as `http://127.0.0.1:10001`, of a server to which all invocations should be sent, with the `Host` header set to the deployment URI. HTTPS proxy URIs are supported, but only HTTP endpoint traffic will be proxied currently. Can be overridden by the `HTTP_PROXY` environment variable. (default: `null`)
</ResponseField>

<ResponseField name="no-proxy" type="array">
    HTTP authorities eg `localhost`, `restate.dev`, `127.0.0.1` that should not be proxied by the http_proxy. Ports are ignored. Subdomains are also matched. An entry “*” matches all hostnames. Can be overridden by the `NO_PROXY` environment variable, which supports comma separated values. (default: `[]`)
...
<ResponseField name="connect-timeout" type="string">
    How long to wait for a TCP connection to be established before considering it a failed attempt. (default: `"10s"`)
</ResponseField>
...
**NOTE**: Setting this value to None (default) users the default recommended value from HTTP2 specs (default: `null`)
</ResponseField>

<ResponseField name="aws-profile" type="string">
    Name of the AWS profile to select. Defaults to 'AWS_PROFILE' env var, or otherwise the `default` profile. (default: `null`)
</ResponseField>

<ResponseField name="aws-assume-role-external-id" type="string">
    An external ID to apply to any AssumeRole operations taken by this client. https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html Can be overridden by the `AWS_EXTERNAL_ID` environment variable. (default: `null`)
</ResponseField>
...
<ResponseField name="gossip-tick-interval" type="string">
    The interval at which the failure detector will tick. Decrease this value for faster reaction to node failures. Note, that every tick comes with an overhead. (default: `"100ms"`)
</ResponseField>

<ResponseField name="gossip-failure-threshold" type="integer">
    Specifies how many gossip intervals of inactivity need to pass before considering a node as dead. (default: `10`)
</ResponseField>

<ResponseField name="gossip-num-peers" type="integer">
    On every gossip interval, how many peers each node attempts to gossip with. The default is optimized for small clusters (less than 5 nodes). On larger clusters, if gossip overhead is noticeable, consider reducing this value to 1. (default: `2`)
</ResponseField>

<ResponseField name="gossip-fd-stability-threshold" type="integer">
    Gossips before failure detector is stable (default: `3`)
</ResponseField>
...
A node becomes a suspect if it has been previously marked as dead for a given generation number. If the node incremented its generation number, it will not be impacted by this threshold. (default: `"5s"`)
</ResponseField>
