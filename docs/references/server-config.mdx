...
<ResponseField name="bind-port" type="integer" post={['default=5122']}>
    Bind port: The network port for the fabric (node-to-node) service. Default is 5122.

</ResponseField>

<ResponseField name="bind-address" type="string" post={[]}>
    Bind address: Combined `bind-ip:bind-port` for the fabric service. This has precedence over the individual `bind-ip` and `bind-port` options.
